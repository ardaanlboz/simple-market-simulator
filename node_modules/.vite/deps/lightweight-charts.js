//#region node_modules/fancy-canvas/size.mjs
function size$1(_a) {
...
//#endregion
//#region node_modules/fancy-canvas/device-pixel-ratio.mjs
...
//#endregion
//#region node_modules/fancy-canvas/canvas-element-bitmap-size.mjs
var DevicePixelContentBoxBinding = function() {
	...
	*/
	...
	DevicePixelContentBoxBinding.prototype._invalidateBitmapSize = function() {
		...
		var newSize = canvasRects[0] !== void 0 ? predictedBitmapSize(canvasRects[0], ratio) : size$1({
...
//#endregion
//#region node_modules/fancy-canvas/canvas-rendering-target.mjs
/**
* @experimental
*/
...
//#endregion
//#region node_modules/lightweight-charts/dist/lightweight-charts.development.mjs
/*!
* @license
...
var customStyleDefaults = { color: "#2196f3" };
...
function setLineStyle(ctx, style) {
	const dashPattern = {
		[0]: [],
		[1]: [ctx.lineWidth, ctx.lineWidth],
		[2]: [2 * ctx.lineWidth, 2 * ctx.lineWidth],
		[3]: [6 * ctx.lineWidth, 6 * ctx.lineWidth],
		[4]: [ctx.lineWidth, 4 * ctx.lineWidth]
	}[style];
	ctx.setLineDash(dashPattern);
}
function drawHorizontalLine(ctx, y, left, right) {
	ctx.beginPath();
	const correction = ctx.lineWidth % 2 ? .5 : 0;
...
function strokeInPixel(ctx, drawFunction) {
	ctx.save();
	if (ctx.lineWidth % 2) ctx.translate(.5, .5);
	drawFunction();
	ctx.restore();
}
/**
...
function normalizeAlphaComponent(component) {
	if (component <= 0 || component > 1) return Math.min(Math.max(component, 0), 1);
...
function colorStringToRgba(colorString) {
	...
	if (colorString in namedColorRgbHexStrings) colorString = namedColorRgbHexStrings[colorString];
	{
...
function rgbaToGrayscale(rgbValue) {
	return .199 * rgbValue[0] + .687 * rgbValue[1] + .114 * rgbValue[2];
}
...
var Delegate = class {
	...
	_internal_unsubscribe(callback) {
		...
		if (index > -1) this._private__listeners.splice(index, 1);
	}
...
function merge(dst, ...sources) {
	for (const src of sources) for (const i in src) {
		if (src[i] === void 0 || !Object.prototype.hasOwnProperty.call(src, i) || [
			"__proto__",
			"constructor",
			"prototype"
		].includes(i)) continue;
		if ("object" !== typeof src[i] || dst[i] === void 0 || Array.isArray(src[i])) dst[i] = src[i];
		else merge(dst[i], src[i]);
	}
	return dst;
}
...
function makeFont(size, family, style) {
	...
	if (family === void 0) family = defaultFontFamily;
...
var PaneRendererMarks = class extends BitmapCoordinatesPaneRenderer {
	...
	_internal__drawImpl({ context: ctx, horizontalPixelRatio, verticalPixelRatio }) {
		if (this._internal__data === null || this._internal__data._internal_visibleRange === null) return;
		const visibleRange = this._internal__data._internal_visibleRange;
		const data = this._internal__data;
		const correction = Math.max(1, Math.floor(horizontalPixelRatio)) % 2 / 2;
...
var CrosshairRenderer = class extends BitmapCoordinatesPaneRenderer {
	...
	_internal__drawImpl({ context: ctx, bitmapSize, horizontalPixelRatio, verticalPixelRatio }) {
		if (this._private__data === null) return;
		...
		if (!vertLinesVisible && !horzLinesVisible) return;
...
var PriceAxisViewRenderer = class {
	...
	_internal_height(rendererOptions, useSecondLine) {
		if (!this._private__data._internal_visible) return 0;
	...
	_internal_draw(target, rendererOptions, textWidthCache, align) {
		if (!this._private__data._internal_visible || this._private__data._internal_text.length === 0) return;
		...
		const geometry = target.useBitmapCoordinateSpace((scope) => {
			...
			if (geom._internal_alignRight) drawRoundRectWithBorder(ctx, gb._internal_xOutside, gb._internal_yTop, gb._internal_totalWidth, gb._internal_totalHeight, backgroundColor, gb._internal_horzBorder, [
				gb._internal_radius,
				0,
				0,
				gb._internal_radius
			], backgroundColor);
			else drawRoundRectWithBorder(ctx, gb._internal_xInside, gb._internal_yTop, gb._internal_totalWidth, gb._internal_totalHeight, backgroundColor, gb._internal_horzBorder, [
				0,
				gb._internal_radius,
				gb._internal_radius,
				0
	...
	_private__calculateGeometry(scope, rendererOptions, textWidthCache, align) {
		...
		const yMidBitmap = Math.round(yMid * verticalPixelRatio) - Math.floor(verticalPixelRatio * .5);
...
var CrosshairPriceAxisView = class extends PriceAxisView {
	...
	_internal__updateRendererData(axisRendererData, paneRendererData, commonRendererData) {
		...
		if (this._private__source._internal_options().mode === 2) return;
		const options = this._private__source._internal_options().horzLine;
		if (!options.labelVisible) return;
		const firstValue = this._private__priceScale._internal_firstValue();
		if (!this._private__source._internal_visible() || this._private__priceScale._internal_isEmpty() || firstValue === null) return;
...
var TimeAxisViewRenderer = class {
	...
	_internal_draw(target, rendererOptions) {
		if (this._private__data === null || this._private__data._internal_visible === false || this._private__data._internal_text.length === 0) return;
		...
		let x1 = Math.floor(coordinate - labelWidthHalf) + .5;
		if (x1 < 0) {
			coordinate = coordinate + Math.abs(0 - x1);
			x1 = Math.floor(coordinate - labelWidthHalf) + .5;
		...
		target.useBitmapCoordinateSpace(({ context: ctx, horizontalPixelRatio, verticalPixelRatio }) => {
			...
			if (data._internal_tickVisible) {
				...
				const tickOffset = Math.floor(horizontalPixelRatio * .5);
...
var CrosshairTimeAxisView = class {
	...
	_private__updateImpl() {
		...
		if (this._private__crosshair._internal_options().mode === 2) return;
		...
		if (timeScale._internal_isEmpty()) return;
...
var Crosshair = class extends DataSource {
	constructor(model, options) {
		...
		const valuePriceProvider = (rawPriceProvider, rawCoordinateProvider) => {
			return (priceScale) => {
				...
				if (priceScale === ensureNotNull(this._private__pane)._internal_defaultPriceScale()) return {
					...
					return {
						_internal_price: priceScale._internal_coordinateToPrice(coordinate, firstValue),
						_internal_coordinate: coordinate
					};
				}
			};
		...
		const valueTimeProvider = (rawIndexProvider, rawCoordinateProvider) => {
			return () => {
				...
				const coordinate = rawCoordinateProvider();
				if (!time || !Number.isFinite(coordinate)) return null;
				return {
					_internal_time: time,
					_internal_coordinate: coordinate
				};
	...
	_internal_setPosition(index, price, pane) {
		if (!this._private__subscribed) this._private__subscribed = true;
		this._private__visible = true;
	...
	_internal_priceAxisViews(pane, priceScale) {
		if (!this._private__visible || this._private__pane !== pane) this._private__priceAxisViews.clear();
		const views = [];
		if (this._private__pane === pane) views.push(this._private__createPriceAxisViewOnDemand(this._private__priceAxisViews, priceScale, this._private__currentPosPriceProvider));
		return views;
	}
	_internal_timeAxisViews() {
	...
	_private__priceScaleByPane(pane) {
		if (pane && !pane._internal_defaultPriceScale()._internal_isEmpty()) return pane._internal_defaultPriceScale();
		return null;
	}
	_private__tryToUpdateViews(index, price, pane) {
		if (this._private__tryToUpdateData(index, price, pane)) this._internal_updateAllViews();
	}
...
function mergePaneInvalidation(beforeValue, newValue) {
	if (beforeValue === void 0) return newValue;
	return {
		_internal_level: Math.max(beforeValue._internal_level, newValue._internal_level),
		_internal_autoScale: beforeValue._internal_autoScale || newValue._internal_autoScale
	};
}
var InvalidateMask = class InvalidateMask {
	...
	_internal_invalidatePane(paneIndex, invalidation) {
		const newValue = mergePaneInvalidation(this._private__invalidatedPanes.get(paneIndex), invalidation);
	...
	_internal_invalidateForPane(paneIndex) {
		...
		if (paneInvalidation === void 0) return { _internal_level: this._private__globalLevel };
		return {
	...
	_internal_setFitContent() {
		...
		this._private__timeScaleInvalidations = [{ _internal_type: 0 }];
	}
	...
	_internal_stopTimeScaleAnimation() {
		this._private__removeTimeScaleAnimation();
		this._private__timeScaleInvalidations.push({ _internal_type: 6 });
	}
	_internal_resetTimeScale() {
		this._internal_stopTimeScaleAnimation();
		this._private__timeScaleInvalidations = [{ _internal_type: 4 }];
	}
	...
	_internal_merge(other) {
		for (const tsInvalidation of other._private__timeScaleInvalidations) this._private__applyTimeScaleInvalidation(tsInvalidation);
	...
	static _internal_light() {
		return new InvalidateMask(2);
	}
	static _internal_full() {
		return new InvalidateMask(3);
	}
	_private__applyTimeScaleInvalidation(invalidation) {
		switch (invalidation._internal_type) {
			...
			case 5:
				...
				break;
			case 6: this._private__removeTimeScaleAnimation();
		}
	}
	_private__removeTimeScaleAnimation() {
		const index = this._private__timeScaleInvalidations.findIndex((inv) => inv._internal_type === 5);
		if (index !== -1) this._private__timeScaleInvalidations.splice(index, 1);
	}
};
...
function numberToStringWithLeadingZero(value, length) {
	...
	if (!isInteger(length)) throw new TypeError("invalid length");
	if (length < 0 || length > 16) throw new TypeError("invalid length");
	if (length === 0) return value.toString();
	return ("0000000000000000" + value.toString()).slice(-length);
}
var PriceFormatter = class {
	constructor(priceScale, minMove) {
		if (!minMove) minMove = 1;
		if (!isNumber(priceScale) || !isInteger(priceScale)) priceScale = 100;
		if (priceScale < 0) throw new TypeError("invalid base");
		this._private__priceScale = priceScale;
	...
	_private__formatAsDecimal(price) {
		...
		if (base > 1) {
			...
			if (fracLength > 0) fracString = formatterOptions._internal_decimalSign + numberToStringWithLeadingZero(0, fracLength);
		}
...
var VolumeFormatter = class {
	...
	format(vol) {
		...
		if (vol < 995) return sign + this._private__formatNumber(vol);
		else if (vol < 999995) return sign + this._private__formatNumber(vol / 1e3) + "K";
	...
	_private__formatNumber(value) {
		...
		if (value >= 1e-15 && value < 1) res = value.toFixed(this._private__precision).replace(/\.?0+$/, "");
		else res = String(value);
		return res.replace(/(\.[1-9]*)0+$/, (e, p1) => p1);
	}
};
function walkLine(renderingScope, items, lineType, visibleRange, barWidth, styleGetter, finishStyledArea) {
	if (items.length === 0 || visibleRange.from >= items.length || visibleRange.to <= 0) return;
	...
	if (visibleRange.to - visibleRange.from < 2) {
		...
		if (currentStyleFirstItem !== currentItem || currentStyleFirstItem === currentItem && lineType === 1) finishStyledArea(renderingScope, currentStyle, currentStyleFirstItem, currentItem);
	}
}
var curveTension = 6;
...
function getControlPoints(points, fromPointIndex, toPointIndex) {
	...
	return [add(points[fromPointIndex], divide(subtract(points[toPointIndex], points[beforeFromPointIndex]), curveTension)), subtract(points[toPointIndex], divide(subtract(points[afterToPointIndex], points[fromPointIndex]), curveTension))];
}
...
var PaneRendererAreaBase = class extends BitmapCoordinatesPaneRenderer {
	...
	_internal__drawImpl(renderingScope) {
		var _a;
		if (this._internal__data === null) return;
...
function isBaseDecimal(value) {
	...
	for (let current = value; current > 1; current /= 10) if (current % 10 !== 0) return false;
	return true;
}
...
var PaneRendererLineBase = class extends BitmapCoordinatesPaneRenderer {
	...
	_internal__drawImpl(renderingScope) {
		...
		if (lineType !== void 0) walkLine(renderingScope, items, lineType, visibleRange, barWidth, styleGetter, finishStyledArea);
		if (pointMarkersRadius) drawSeriesPointMarkers(renderingScope, items, pointMarkersRadius, visibleRange, styleGetter);
	}
};
...
function visibleTimedValues(items, range, extendedRange) {
	...
	let extendedTo = to;
	if (from > 0 && from < items.length && items[from]._internal_time >= firstBar) extendedFrom = from - 1;
	if (to > 0 && to < items.length && items[to - 1]._internal_time <= lastBar) extendedTo = to + 1;
	return {
		from: extendedFrom,
		to: extendedTo
	};
...
var SeriesPaneViewBase = class {
	...
	_internal_update(updateType) {
		this._internal__invalidated = true;
		if (updateType === "data") this._internal__dataInvalidated = true;
		if (updateType === "options") this._internal__optionsInvalidated = true;
	}
	_internal_renderer() {
		if (!this._internal__series._internal_visible()) return null;
	...
	_private__makeValidImpl() {
		...
		if (timeScale._internal_isEmpty() || priceScale._internal_isEmpty()) return;
		const visibleBars = timeScale._internal_visibleStrictRange();
		if (visibleBars === null) return;
		if (this._internal__series._internal_bars()._internal_size() === 0) return;
...
var LinePaneViewBase = class extends SeriesPaneViewBase {
	...
	_internal__fillRawPoints() {
		...
		this._internal__items = this._internal__series._internal_bars()._internal_rows().map((row) => {
			const value = row._internal_value[3];
...
function optimalBarWidth(barSpacing, pixelRatio) {
	return Math.floor(barSpacing * .3 * pixelRatio);
}
function optimalCandlestickWidth(barSpacing, pixelRatio) {
	const barSpacingSpecialCaseFrom = 2.5;
	const barSpacingSpecialCaseTo = 4;
	const barSpacingSpecialCaseCoeff = 3;
	if (barSpacing >= barSpacingSpecialCaseFrom && barSpacing <= barSpacingSpecialCaseTo) return Math.floor(barSpacingSpecialCaseCoeff * pixelRatio);
	const coeff = 1 - .2 * Math.atan(Math.max(barSpacingSpecialCaseTo, barSpacing) - barSpacingSpecialCaseTo) / (Math.PI * .5);
...
var PaneRendererBars = class extends BitmapCoordinatesPaneRenderer {
	...
	_internal__drawImpl({ context: ctx, horizontalPixelRatio, verticalPixelRatio }) {
		if (this._private__data === null || this._private__data._internal_bars.length === 0 || this._private__data._internal_visibleRange === null) return;
		this._private__barWidth = this._private__calcBarWidth(horizontalPixelRatio);
		if (this._private__barWidth >= 2) {
			if (Math.max(1, Math.floor(horizontalPixelRatio)) % 2 !== this._private__barWidth % 2) this._private__barWidth--;
		}
		...
		for (let i = this._private__data._internal_visibleRange.from; i < this._private__data._internal_visibleRange.to; ++i) {
			...
			const bodyWidthHalf = Math.floor(this._private__barLineWidth * .5);
	...
	_private__calcBarWidth(pixelRatio) {
		return Math.max(Math.floor(pixelRatio), Math.floor(optimalBarWidth(ensureNotNull(this._private__data)._internal_barSpacing, pixelRatio)));
	}
};
var BarsPaneViewBase = class extends SeriesPaneViewBase {
	...
	_internal__createDefaultItem(time, bar, colorer) {
		return {
			_internal_time: time,
			_internal_open: bar._internal_value[0],
			_internal_high: bar._internal_value[1],
			_internal_low: bar._internal_value[2],
			_internal_close: bar._internal_value[3],
			_internal_x: NaN,
			_internal_openY: NaN,
			_internal_highY: NaN,
			_internal_lowY: NaN,
...
var PaneRendererCandlesticks = class extends BitmapCoordinatesPaneRenderer {
	...
	_internal__drawImpl(renderingScope) {
		...
		if (this._private__barWidth >= 2) {
			if (Math.floor(horizontalPixelRatio) % 2 !== this._private__barWidth % 2) this._private__barWidth--;
		}
		const bars = this._private__data._internal_bars;
		if (this._private__data._internal_wickVisible) this._private__drawWicks(renderingScope, bars, this._private__data._internal_visibleRange);
		if (this._private__data._internal_borderVisible) this._private__drawBorder(renderingScope, bars, this._private__data._internal_visibleRange);
		const borderWidth = this._private__calculateBorderWidth(horizontalPixelRatio);
		if (!this._private__data._internal_borderVisible || this._private__barWidth > borderWidth * 2) this._private__drawCandles(renderingScope, bars, this._private__data._internal_visibleRange);
	}
	_private__drawWicks(renderingScope, bars, visibleRange) {
		...
		const wickOffset = Math.floor(wickWidth * .5);
		let prevEdge = null;
		for (let i = visibleRange.from; i < visibleRange.to; i++) {
			...
			let left = Math.round(horizontalPixelRatio * bar._internal_x) - wickOffset;
	...
	_private__calculateBorderWidth(pixelRatio) {
		...
		if (this._private__barWidth <= 2 * borderWidth) borderWidth = Math.floor((this._private__barWidth - 1) * .5);
		const res = Math.max(Math.floor(pixelRatio), borderWidth);
		if (this._private__barWidth <= res * 2) return Math.max(Math.floor(pixelRatio), Math.floor(1 * pixelRatio));
		return res;
	}
	_private__drawBorder(renderingScope, bars, visibleRange) {
		...
		for (let i = visibleRange.from; i < visibleRange.to; i++) {
			...
			let left = Math.round(bar._internal_x * horizontalPixelRatio) - Math.floor(this._private__barWidth * .5);
			...
			if (this._private__data._internal_barSpacing * horizontalPixelRatio > 2 * borderWidth) fillRectInnerBorder(ctx, left, top, right - left + 1, bottom - top + 1, borderWidth);
			else {
				const width = right - left + 1;
...
var PaneRendererHistogram = class extends BitmapCoordinatesPaneRenderer {
	...
	_internal__drawImpl({ context: ctx, horizontalPixelRatio, verticalPixelRatio }) {
		if (this._private__data === null || this._private__data._internal_items.length === 0 || this._private__data._internal_visibleRange === null) return;
		if (!this._private__precalculatedCache.length) this._private__fillPrecalculatedCache(horizontalPixelRatio);
		const tickWidth = Math.max(1, Math.floor(verticalPixelRatio));
		const topHistogramBase = Math.round(this._private__data._internal_histogramBase * verticalPixelRatio) - Math.floor(tickWidth / 2);
	...
	_private__fillPrecalculatedCache(pixelRatio) {
		...
		for (let i = this._private__data._internal_visibleRange.from + 1; i < this._private__data._internal_visibleRange.to; i++) {
			...
			if (current._internal_time !== prev._internal_time + 1) continue;
			if (current._internal_left - prev._internal_right !== spacing + 1) if (prev._internal_roundedCenter > prev._internal_center) prev._internal_right = current._internal_left - spacing - 1;
			else current._internal_left = prev._internal_right + spacing + 1;
		}
		let minWidth = Math.ceil(this._private__data._internal_barSpacing * pixelRatio);
		for (let i = this._private__data._internal_visibleRange.from; i < this._private__data._internal_visibleRange.to; i++) {
			const current = this._private__precalculatedCache[i - this._private__data._internal_visibleRange.from];
			if (current._internal_right < current._internal_left) current._internal_right = current._internal_left;
			const width = current._internal_right - current._internal_left + 1;
			minWidth = Math.min(width, minWidth);
		}
		if (spacing > 0 && minWidth < alignToMinimalWidthLimit) for (let i = this._private__data._internal_visibleRange.from; i < this._private__data._internal_visibleRange.to; i++) {
			const current = this._private__precalculatedCache[i - this._private__data._internal_visibleRange.from];
			if (current._internal_right - current._internal_left + 1 > minWidth) if (current._internal_roundedCenter > current._internal_center) current._internal_right -= 1;
			else current._internal_left += 1;
		}
	}
};
...
var TextWidthCache = class {
	...
	_private__getMetrics(ctx, text, optimizationReplacementRe) {
		...
		if (this._private__cache.has(cacheString)) return ensureDefined(this._private__cache.get(cacheString))._internal_metrics;
		...
		ctx.restore();
		if (metrics.width === 0 && !!text.length) return metrics;
...
var PanePriceAxisViewRenderer = class {
	...
	_internal_draw(target) {
		if (this._private__rendererOptions === null || this._private__priceAxisViewRenderer === null) return;
...
var PanePriceAxisView = class {
	...
	_internal_renderer() {
		...
		if (position === "overlay") return null;
...
var HorizontalLineRenderer = class extends BitmapCoordinatesPaneRenderer {
	...
	_internal_hitTest(x, y) {
		var _a;
		if (!((_a = this._private__data) === null || _a === void 0 ? void 0 : _a._internal_visible)) return null;
		const { _internal_y: itemY, _internal_lineWidth: lineWidth, _internal_externalId: externalId } = this._private__data;
		if (y >= itemY - lineWidth - 7 && y <= itemY + lineWidth + 7) return {
	...
	_internal__drawImpl({ context: ctx, bitmapSize, horizontalPixelRatio, verticalPixelRatio }) {
		if (this._private__data === null) return;
		if (this._private__data._internal_visible === false) return;
		const y = Math.round(this._private__data._internal_y * verticalPixelRatio);
...
var SeriesHorizontalBaseLinePaneView = class extends SeriesHorizontalLinePaneView {
	...
	_internal__updateImpl() {
		...
		if (!seriesOptions.baseLineVisible || !this._internal__series._internal_visible()) return;
		const firstValue = this._internal__series._internal_firstValue();
		if (firstValue === null) return;
...
var animationStagesData = [
	{
		...
		_internal_startFillAlpha: .25,
		_internal_endFillAlpha: 0,
		_internal_startStrokeAlpha: .4,
		_internal_endStrokeAlpha: .8
	},
	{
		_internal_start: .25,
		_internal_end: .525,
...
function color(seriesLineColor, stage, startAlpha, endAlpha) {
	return applyAlpha(seriesLineColor, startAlpha + (endAlpha - startAlpha) * stage);
}
...
function animationData(durationSinceStart, lineColor) {
	...
	for (const stageData of animationStagesData) if (globalStage >= stageData._internal_start && globalStage <= stageData._internal_end) {
...
var SeriesLastPriceAnimationPaneView = class {
	...
	_internal_onNewRealtimeDataReceived() {
		...
		if (this._private__series._internal_options().lastPriceAnimation === 2) {
			...
			if (timeToAnimationEnd > 0) {
				if (timeToAnimationEnd < 2600 / 4) this._private__endTime += 2600;
				return;
			}
	...
	_internal_animationActive() {
		switch (this._private__series._internal_options().lastPriceAnimation) {
			case 0: return false;
			case 1: return true;
			case 2: return performance.now() <= this._private__endTime;
		}
	}
	_internal_renderer() {
		if (this._private__invalidated) {
	...
	_private__updateImpl() {
		...
		if (visibleRange === null || firstValue === null) return;
		const lastValue = this._private__series._internal_lastValueData(true);
		if (lastValue._internal_noData || !visibleRange._internal_contains(lastValue._internal_index)) return;
	...
	_private__duration() {
		return this._internal_animationActive() ? performance.now() - this._private__startTime : 2599;
	}
};
...
function shapeSize(shape, originalSize) {
	switch (shape) {
		case "arrowDown":
		case "arrowUp": return size(originalSize, 1);
		case "circle": return size(originalSize, .8);
		case "square": return size(originalSize, .7);
	}
}
...
function shapeMargin(barSpacing) {
	return Math.max(size(barSpacing, .1), 3);
}
function calculateAdjustedMargin(margin, hasSide, hasInBar) {
	if (hasSide) return margin;
	else if (hasInBar) return Math.ceil(margin / 2);
	return 0;
}
...
function drawArrow(up, ctx, coords, size) {
	const halfArrowSize = (shapeSize("arrowUp", size) - 1) / 2 * coords._internal_pixelRatio;
	const halfBaseSize = (ceiledOdd(size / 2) - 1) / 2 * coords._internal_pixelRatio;
	ctx.beginPath();
	if (up) {
...
function drawCircle(ctx, coords, size) {
	const halfSize = (shapeSize("circle", size) - 1) / 2;
	ctx.beginPath();
...
function hitTestCircle(centerX, centerY, size, x, y) {
	const tolerance = 2 + shapeSize("circle", size) / 2;
	const xOffset = centerX - x;
	const yOffset = centerY - y;
	return Math.sqrt(xOffset * xOffset + yOffset * yOffset) <= tolerance;
}
...
var SeriesMarkersRenderer = class extends BitmapCoordinatesPaneRenderer {
	...
	_internal_hitTest(x, y) {
		if (this._private__data === null || this._private__data._internal_visibleRange === null) return null;
		for (let i = this._private__data._internal_visibleRange.from; i < this._private__data._internal_visibleRange.to; i++) {
			const item = this._private__data._internal_items[i];
			if (hitTestItem(item, x, y)) return {
	...
	_internal__drawImpl({ context: ctx, horizontalPixelRatio, verticalPixelRatio }, isHovered, hitTestData) {
		if (this._private__data === null || this._private__data._internal_visibleRange === null) return;
		ctx.textBaseline = "middle";
...
function drawItem(item, ctx, horizontalPixelRatio, verticalPixelRatio) {
	...
	if (item._internal_text !== void 0) drawText(ctx, item._internal_text._internal_content, item._internal_text._internal_x, item._internal_text._internal_y, horizontalPixelRatio, verticalPixelRatio);
...
function drawShape(item, ctx, coordinates) {
	if (item._internal_size === 0) return;
...
function hitTestItem(item, x, y) {
	if (item._internal_text !== void 0 && hitTestText(item._internal_text._internal_x, item._internal_text._internal_y, item._internal_text._internal_width, item._internal_text._internal_height, x, y)) return true;
	return hitTestShape(item, x, y);
}
function hitTestShape(item, x, y) {
	if (item._internal_size === 0) return false;
	switch (item._internal_shape) {
		case "arrowDown": return hitTestArrow(true, item._internal_x, item._internal_y, item._internal_size, x, y);
		case "arrowUp": return hitTestArrow(false, item._internal_x, item._internal_y, item._internal_size, x, y);
		case "circle": return hitTestCircle(item._internal_x, item._internal_y, item._internal_size, x, y);
		case "square": return hitTestSquare(item._internal_x, item._internal_y, item._internal_size, x, y);
	}
}
function fillSizeAndY(rendererItem, marker, seriesData, offsets, textHeight, shapeMargin, priceScale, timeScale, firstValue) {
	...
	switch (marker.position) {
		case "inBar":
			...
			if (rendererItem._internal_text !== void 0) rendererItem._internal_text._internal_y = rendererItem._internal_y + halfSize + shapeMargin + textHeight * .6;
			return;
		case "aboveBar":
			...
			if (rendererItem._internal_text !== void 0) {
				rendererItem._internal_text._internal_y = rendererItem._internal_y - halfSize - textHeight * .6;
				offsets._internal_aboveBar += textHeight * 1.2;
			}
		...
		case "belowBar":
			...
			if (rendererItem._internal_text !== void 0) {
				rendererItem._internal_text._internal_y = rendererItem._internal_y + halfSize + shapeMargin + textHeight * .6;
				offsets._internal_belowBar += textHeight * 1.2;
			}
...
var SeriesMarkersPaneView = class {
	...
	_internal_renderer(addAnchors) {
		if (!this._private__series._internal_visible()) return null;
		if (this._private__invalidated) this._internal__makeValid();
	...
	_internal_autoScaleMargins() {
		if (this._private__autoScaleMarginsInvalidated) {
			...
			} else this._private__autoScaleMargins = null;
			this._private__autoScaleMarginsInvalidated = false;
		}
		return this._private__autoScaleMargins;
	}
	_internal__getMarkerPositions() {
		if (this._private__markersPositions === null) this._private__markersPositions = this._private__series._internal_indexedMarkers().reduce((acc, marker) => {
			if (!acc[marker.position]) acc[marker.position] = true;
			return acc;
		}, {
			inBar: false,
			aboveBar: false,
	...
	_internal__makeValid() {
		...
		const firstValue = this._private__series._internal_firstValue();
		if (firstValue === null) return;
		if (this._private__data._internal_items.length === 0) return;
		...
		for (let index = this._private__data._internal_visibleRange.from; index < this._private__data._internal_visibleRange.to; index++) {
			...
			if (marker.text !== void 0 && marker.text.length > 0) rendererItem._internal_text = {
...
var SeriesPriceLinePaneView = class extends SeriesHorizontalLinePaneView {
	...
	_internal__updateImpl() {
		...
		data._internal_visible = false;
		const seriesOptions = this._internal__series._internal_options();
		if (!seriesOptions.priceLineVisible || !this._internal__series._internal_visible()) return;
		const lastValueData = this._internal__series._internal_lastValueData(seriesOptions.priceLineSource === 0);
		if (lastValueData._internal_noData) return;
		data._internal_visible = true;
...
var SeriesPriceAxisView = class extends PriceAxisView {
	...
	_internal__updateRendererData(axisRendererData, paneRendererData, commonRendererData) {
		...
		if (!source._internal_visible()) return;
	...
	_internal__paneText(lastValue, showSeriesLastValue, showSymbolLabel, showPriceAndPercentage) {
		...
		if (showSymbolLabel && title.length !== 0) result += `${title} `;
		if (showSeriesLastValue && showPriceAndPercentage) result += this._private__source._internal_priceScale()._internal_isPercentage() ? lastValue._internal_formattedPriceAbsolute : lastValue._internal_formattedPricePercentage;
		return result.trim();
	}
	_internal__axisText(lastValueData, showSeriesLastValue, showPriceAndPercentage) {
		if (!showSeriesLastValue) return "";
		if (!showPriceAndPercentage) return lastValueData._internal_text;
...
function computeFiniteResult(method, valueOne, valueTwo, fallback) {
	...
	if (firstFinite && secondFinite) return method(valueOne, valueTwo);
	return !firstFinite && !secondFinite ? fallback : firstFinite ? valueOne : valueTwo;
}
var PriceRangeImpl = class PriceRangeImpl {
	...
	_internal_merge(anotherRange) {
		if (anotherRange === null) return this;
	...
	_internal_scaleAroundCenter(coeff) {
		...
		if (this._private__maxValue - this._private__minValue === 0) return;
		const center = (this._private__maxValue + this._private__minValue) * .5;
...
var AutoscaleInfoImpl = class AutoscaleInfoImpl {
	...
	_internal_toRaw() {
		if (this._private__priceRange === null) return null;
		return {
...
var CustomPriceLinePaneView = class extends SeriesHorizontalLinePaneView {
	...
	_internal__updateImpl() {
		...
		if (!this._internal__series._internal_visible() || !lineOptions.lineVisible) return;
...
var CustomPriceLinePriceAxisView = class extends PriceAxisView {
	...
	_internal__updateRendererData(axisRendererData, paneRendererData, commonData) {
		...
		if (!labelVisible || !series._internal_visible()) return;
		const y = this._private__priceLine._internal_yCoord();
		if (y === null) return;
		if (showPaneLabel) {
...
var CustomPriceLine = class {
	...
	_internal_yCoord() {
		...
		if (series._internal_model()._internal_timeScale()._internal_isEmpty() || priceScale._internal_isEmpty()) return null;
		const firstValue = series._internal_firstValue();
		if (firstValue === null) return null;
...
var barStyleFnMap = {
	Bar: (findBar, barStyle, barIndex, precomputedBars) => {
		...
		return { _internal_barColor: (_a = currentBar._internal_color) !== null && _a !== void 0 ? _a : isUp ? upColor : downColor };
	},
	...
	Custom: (findBar, customStyle, barIndex, precomputedBars) => {
		var _a;
		return { _internal_barColor: (_a = ensureNotNull(findBar(barIndex, precomputedBars))._internal_color) !== null && _a !== void 0 ? _a : customStyle.color };
	},
	...
	Baseline: (findBar, baselineStyle, barIndex, precomputedBars) => {
		...
		const currentBar = ensureNotNull(findBar(barIndex, precomputedBars));
		return {
			_internal_barColor: currentBar._internal_value[3] >= baselineStyle.baseValue.price ? baselineStyle.topLineColor : baselineStyle.bottomLineColor,
	...
	Histogram: (findBar, histogramStyle, barIndex, precomputedBars) => {
		var _a;
		return { _internal_barColor: (_a = ensureNotNull(findBar(barIndex, precomputedBars))._internal_color) !== null && _a !== void 0 ? _a : histogramStyle.color };
	}
};
var SeriesBarColorer = class {
	constructor(series) {
		this._private__findBar = (barIndex, precomputedBars) => {
			if (precomputedBars !== void 0) return precomputedBars._internal_value;
...
var PlotList = class {
	...
	_internal_contains(index) {
		return this._private__search(index, 0) !== null;
	}
	...
	_internal_minMaxOnRangeCached(start, end, plots) {
		if (this._internal_isEmpty()) return null;
		let result = null;
	...
	_private__search(index, searchMode) {
		...
		if (exactPos === null && searchMode !== 0) switch (searchMode) {
			case -1: return this._private__searchNearestLeft(index);
			case 1: return this._private__searchNearestRight(index);
	...
	_private__searchNearestLeft(index) {
		...
		if (nearestLeftPos > 0) nearestLeftPos = nearestLeftPos - 1;
	...
	_private__bsearch(index) {
		...
		if (start !== this._private__items.length && !(index < this._private__items[start]._internal_index)) return start;
		return null;
	}
	...
	_private__plotMinMax(startIndex, endIndexExclusive, plotIndex) {
		...
		for (let i = startIndex; i < endIndexExclusive; i++) {
			const v = this._private__items[i]._internal_value[plotIndex];
			...
			if (result === null) result = {
				...
				if (v < result._internal_min) result._internal_min = v;
				if (v > result._internal_max) result._internal_max = v;
			}
		}
		return result;
	}
...
function mergeMinMax(first, second) {
	if (first === null) return second;
		...
		_internal_min: Math.min(first._internal_min, second._internal_min),
		_internal_max: Math.max(first._internal_max, second._internal_max)
	};
}
...
var SeriesPrimitiveRendererWrapper = class {
	...
	_internal_drawBackground(target, isHovered, hitTestData) {
		var _a, _b;
		(_b = (_a = this._private__baseRenderer).drawBackground) === null || _b === void 0 || _b.call(_a, target);
	}
};
var SeriesPrimitivePaneViewWrapper = class {
	...
	_internal_renderer() {
		...
		if (baseRenderer === null) return null;
		if (((_a = this._private__cache) === null || _a === void 0 ? void 0 : _a._internal_base) === baseRenderer) return this._private__cache._internal_wrapper;
...
var SeriesPrimitiveWrapper = class {
	...
	_internal_updateAllViews() {
		var _a, _b;
		(_b = (_a = this._private__primitive).updateAllViews) === null || _b === void 0 || _b.call(_a);
	}
	_internal_paneViews() {
		var _a, _b, _c, _d;
		const base = (_c = (_b = (_a = this._private__primitive).paneViews) === null || _b === void 0 ? void 0 : _b.call(_a)) !== null && _c !== void 0 ? _c : [];
		if (((_d = this._private__paneViewsCache) === null || _d === void 0 ? void 0 : _d._internal_base) === base) return this._private__paneViewsCache._internal_wrapper;
	...
	_internal_timeAxisViews() {
		...
		if (((_d = this._private__timeAxisViewsCache) === null || _d === void 0 ? void 0 : _d._internal_base) === base) return this._private__timeAxisViewsCache._internal_wrapper;
		const timeScale = this._private__series._internal_model()._internal_timeScale();
	...
	_internal_priceAxisViews() {
		...
		if (((_d = this._private__priceAxisViewsCache) === null || _d === void 0 ? void 0 : _d._internal_base) === base) return this._private__priceAxisViewsCache._internal_wrapper;
		const priceScale = this._private__series._internal_priceScale();
	...
	_internal_priceAxisPaneViews() {
		...
		if (((_d = this._private__priceAxisPaneViewsCache) === null || _d === void 0 ? void 0 : _d._internal_base) === base) return this._private__priceAxisPaneViewsCache._internal_wrapper;
		const wrapper = base.map((pw) => new SeriesPrimitivePaneViewWrapper(pw));
	...
	_internal_timeAxisPaneViews() {
		...
		if (((_d = this._private__timeAxisPaneViewsCache) === null || _d === void 0 ? void 0 : _d._internal_base) === base) return this._private__timeAxisPaneViewsCache._internal_wrapper;
		const wrapper = base.map((pw) => new SeriesPrimitivePaneViewWrapper(pw));
...
function extractPrimitivePaneViews(primitives, extractor, zOrder, destination) {
	primitives.forEach((wrapper) => {
		extractor(wrapper).forEach((paneView) => {
			if (paneView._internal_zOrder() !== zOrder) return;
...
var Series = class extends PriceDataSource {
	constructor(model, options, seriesType, pane, customPaneView) {
		...
		if (seriesType === "Area" || seriesType === "Line" || seriesType === "Baseline") this._private__lastPriceAnimationPaneView = new SeriesLastPriceAnimationPaneView(this);
		this._private__recreateFormatter();
		this._private__recreatePaneViews(customPaneView);
	}
	_internal_destroy() {
		if (this._private__animationTimeoutId !== null) clearTimeout(this._private__animationTimeoutId);
	}
	...
	_internal_lastValueData(globalLast) {
		...
		if (this._internal_model()._internal_timeScale()._internal_isEmpty() || priceScale._internal_isEmpty() || this._private__data._internal_isEmpty()) return noDataRes;
		...
		if (visibleBars === null || firstValue === null) return noDataRes;
		let bar;
		let lastIndex;
		if (globalLast) {
			const lastBar = this._private__data._internal_last();
			if (lastBar === null) return noDataRes;
			bar = lastBar;
			lastIndex = lastBar._internal_index;
		} else {
			const endBar = this._private__data._internal_search(visibleBars._internal_right(), -1);
			if (endBar === null) return noDataRes;
			bar = this._private__data._internal_valueAt(endBar._internal_index);
			if (bar === null) return noDataRes;
			lastIndex = endBar._internal_index;
		}
		const price = bar._internal_value[3];
		const style = this._internal_barColorer()._internal_barStyle(lastIndex, { _internal_value: bar });
	...
	_internal_barColorer() {
		if (this._private__barColorerCache !== null) return this._private__barColorerCache;
	...
	_internal_applyOptions(options) {
		...
		if (targetPriceScaleId !== void 0 && targetPriceScaleId !== this._private__options.priceScaleId) this._internal_model()._internal_moveSeriesToScale(this, targetPriceScaleId);
		merge(this._private__options, options);
	...
	_internal_setData(data, updateInfo) {
		...
		if (this._private__lastPriceAnimationPaneView !== null) {
			if (updateInfo && updateInfo._internal_lastBarUpdatedOrNewBarsAddedToTheRight) this._private__lastPriceAnimationPaneView._internal_onNewRealtimeDataReceived();
			else if (data.length === 0) this._private__lastPriceAnimationPaneView._internal_onDataCleared();
		}
	...
	_internal_removePriceLine(line) {
		...
		if (index !== -1) this._private__customPriceLines.splice(index, 1);
		this._internal_model()._internal_updateSource(this);
	}
	...
	_internal_firstValue() {
		...
		return {
			_internal_value: bar._internal_value[3],
	...
	_internal_firstBar() {
		...
		return this._private__data._internal_search(startTimePoint, 1);
	}
	_internal_bars() {
		return this._private__data;
	}
	_internal_dataAt(time) {
		...
		if (this._private__seriesType === "Bar" || this._private__seriesType === "Candlestick" || this._private__seriesType === "Custom") return {
			_internal_open: prices._internal_value[0],
			_internal_high: prices._internal_value[1],
			_internal_low: prices._internal_value[2],
			_internal_close: prices._internal_value[3]
		};
		else return prices._internal_value[3];
	}
	_internal_topPaneViews(pane) {
		...
		if (animationPaneView === null || !animationPaneView._internal_visible()) return res;
		if (this._private__animationTimeoutId === null && animationPaneView._internal_animationActive()) this._private__animationTimeoutId = setTimeout(() => {
			this._private__animationTimeoutId = null;
	...
	_internal_paneViews() {
		const res = [];
		if (!this._private__isOverlay()) res.push(this._private__baseHorizontalLineView);
	...
	_internal_priceAxisViews(pane, priceScale) {
		if (priceScale !== this._internal__priceScale && !this._private__isOverlay()) return [];
		const result = [...this._private__priceAxisViews];
		for (const customPriceLine of this._private__customPriceLines) result.push(customPriceLine._internal_priceAxisView());
	...
	_internal_updateAllViews() {
		...
		this._private__paneView._internal_update();
		this._private__markersPaneView._internal_update();
		for (const priceAxisView of this._private__priceAxisViews) priceAxisView._internal_update();
		for (const customPriceLine of this._private__customPriceLines) customPriceLine._internal_update();
		this._private__priceLineView._internal_update();
		this._private__baseHorizontalLineView._internal_update();
		(_a = this._private__lastPriceAnimationPaneView) === null || _a === void 0 || _a._internal_update();
	...
	_internal_markerDataAtIndex(index) {
		if (!((this._private__seriesType === "Line" || this._private__seriesType === "Area" || this._private__seriesType === "Baseline") && this._private__options.crosshairMarkerVisible)) return null;
		const bar = this._private__data._internal_valueAt(index);
		if (bar === null) return null;
		return {
			_internal_price: bar._internal_value[3],
			_internal_radius: this._private__markerRadius(),
			_internal_borderColor: this._private__markerBorderColor(),
			_internal_borderWidth: this._private__markerBorderWidth(),
			_internal_backgroundColor: this._private__markerBackgroundColor(index)
		};
	}
	_internal_title() {
	...
	_internal_customSeriesPlotValuesBuilder() {
		if (this._private__paneView instanceof SeriesCustomPaneView === false) return;
		return (data) => {
	...
	_private__isOverlay() {
		return !isDefaultPriceScale(this._internal_priceScale()._internal_id());
	}
	_private__autoscaleInfoImpl(startTimePoint, endTimePoint) {
		if (!isInteger(startTimePoint) || !isInteger(endTimePoint) || this._private__data._internal_isEmpty()) return null;
		const plots = this._private__seriesType === "Line" || this._private__seriesType === "Area" || this._private__seriesType === "Baseline" || this._private__seriesType === "Histogram" ? [3] : [2, 1];
		...
		this._private__primitives.forEach((primitive) => {
			...
			if (primitiveAutoscale === null || primitiveAutoscale === void 0 ? void 0 : primitiveAutoscale.margins) margins = mergeMargins(margins, primitiveAutoscale.margins);
		});
	...
	_private__markerRadius() {
		switch (this._private__seriesType) {
			case "Line":
			case "Area":
			case "Baseline": return this._private__options.crosshairMarkerRadius;
		}
		return 0;
	}
	_private__markerBorderColor() {
		switch (this._private__seriesType) {
			...
			case "Baseline": {
				...
				if (crosshairMarkerBorderColor.length !== 0) return crosshairMarkerBorderColor;
			}
		}
		return null;
	}
	_private__markerBorderWidth() {
		switch (this._private__seriesType) {
			case "Line":
			case "Area":
			case "Baseline": return this._private__options.crosshairMarkerBorderWidth;
		}
		return 0;
	}
	_private__markerBackgroundColor(index) {
		switch (this._private__seriesType) {
			...
			case "Baseline": {
				...
				if (crosshairMarkerBackgroundColor.length !== 0) return crosshairMarkerBackgroundColor;
			}
		}
	...
	_private__recreateFormatter() {
		...
		if (this._internal__priceScale !== null) this._internal__priceScale._internal_updateFormatter();
	}
	_private__recalculateMarkers() {
		...
		this._private__indexedMarkers = this._private__markers.map((marker, index) => {
			...
			return {
				time: ensureNotNull(this._private__data._internal_search(timePointIndex, searchMode))._internal_index,
...
var Magnet = class {
	...
	_internal_align(price, index, pane) {
		let res = price;
		if (this._private__options.mode === 0) return res;
		const defaultPriceScale = pane._internal_defaultPriceScale();
		const firstValue = defaultPriceScale._internal_firstValue();
		if (firstValue === null) return res;
		const y = defaultPriceScale._internal_priceToCoordinate(price, firstValue);
		const candidates = pane._internal_dataSources().filter(((ds) => ds instanceof Series)).reduce((acc, series) => {
			if (pane._internal_isOverlay(series) || !series._internal_visible()) return acc;
			const ps = series._internal_priceScale();
			const bars = series._internal_bars();
			if (ps._internal_isEmpty() || !bars._internal_contains(index)) return acc;
			const bar = bars._internal_valueAt(index);
			if (bar === null) return acc;
			const firstPrice = ensure(series._internal_firstValue());
			return acc.concat([ps._internal_priceToCoordinate(bar._internal_value[3], firstPrice._internal_value)]);
		}, []);
...
function toPercentRange(priceRange, baseValue) {
	return new PriceRangeImpl(toPercent(priceRange._internal_minValue(), baseValue), toPercent(priceRange._internal_maxValue(), baseValue));
}
...
function toIndexedTo100Range(priceRange, baseValue) {
	return new PriceRangeImpl(toIndexedTo100(priceRange._internal_minValue(), baseValue), toIndexedTo100(priceRange._internal_maxValue(), baseValue));
}
...
function convertPriceRangeToLog(priceRange, logFormula) {
	...
	return new PriceRangeImpl(toLog(priceRange._internal_minValue(), logFormula), toLog(priceRange._internal_maxValue(), logFormula));
}
...
function convertPriceRangeFromLog(priceRange, logFormula) {
	if (priceRange === null) return null;
	return new PriceRangeImpl(fromLog(priceRange._internal_minValue(), logFormula), fromLog(priceRange._internal_maxValue(), logFormula));
}
function logFormulaForPriceRange(range) {
	if (range === null) return defLogFormula;
	const diff = Math.abs(range._internal_maxValue() - range._internal_minValue());
	if (diff >= 1 || diff < 1e-15) return defLogFormula;
	...
	return {
		...
		_internal_coordOffset: 1 / Math.pow(10, logicalOffset)
	};
}
...
var PriceTickSpanCalculator = class {
	constructor(base, integralDividers) {
		...
		if (isBaseDecimal(this._private__base)) this._private__fractionalDividers = [
			2,
			2.5,
			2
		];
			...
			for (let baseRest = this._private__base; baseRest !== 1;) {
				if (baseRest % 2 === 0) {
					...
					baseRest /= 5;
				} else throw new Error("unexpected base");
				if (this._private__fractionalDividers.length > 100) throw new Error("something wrong with base");
			}
		}
	}
	_internal_tickSpan(high, low, maxTickSpan) {
		...
		while (true) {
			const resultTickSpanLargerMinMovement = greaterOrEqual(resultTickSpan, minMovement, 1e-14) && resultTickSpan > minMovement + 1e-14;
			const resultTickSpanLargerMaxTickSpan = greaterOrEqual(resultTickSpan, maxTickSpan * c, 1e-14);
			const resultTickSpanLarger1 = greaterOrEqual(resultTickSpan, 1, 1e-14);
			if (!(resultTickSpanLargerMinMovement && resultTickSpanLargerMaxTickSpan && resultTickSpanLarger1)) break;
			resultTickSpan /= c;
			c = this._private__integralDividers[++index % this._private__integralDividers.length];
		}
		if (resultTickSpan <= minMovement + 1e-14) resultTickSpan = minMovement;
		resultTickSpan = Math.max(1, resultTickSpan);
		if (this._private__fractionalDividers.length > 0 && equal(resultTickSpan, 1, 1e-14)) {
			index = 0;
			c = this._private__fractionalDividers[0];
			while (greaterOrEqual(resultTickSpan, maxTickSpan * c, 1e-14) && resultTickSpan > minMovement + 1e-14) {
				resultTickSpan /= c;
...
var PriceTickMarkBuilder = class {
	...
	_internal_rebuildTickMarks() {
		...
		for (let logical = high - mod; logical > low; logical -= span) {
			...
			if (prevCoord !== null && Math.abs(coord - prevCoord) < this._private__tickMarkHeight()) continue;
			if (coord < minCoord || coord > maxCoord) continue;
			...
			} else this._private__marks.push({
				_internal_coord: coord,
				_internal_label: priceScale._internal_formatLogical(logical)
			});
			targetIndex++;
			prevCoord = coord;
			if (priceScale._internal_isLog()) span = this._internal_tickSpan(logical * sign, low);
		}
...
var PriceScale = class {
	...
	_internal_applyOptions(options) {
		...
		if (options.mode !== void 0) this._internal_setMode({ _internal_mode: options.mode });
		if (options.scaleMargins !== void 0) {
			...
			if (top < 0 || top > 1) throw new Error(`Invalid top margin - expect value between 0 and 1, given=${top}`);
			if (bottom < 0 || bottom > 1) throw new Error(`Invalid bottom margin - expect value between 0 and 1, given=${bottom}`);
			if (top + bottom > 1) throw new Error(`Invalid margins - sum of margins must be less than 1, given=${top + bottom}`);
	...
	_internal_setMode(newMode) {
		...
		let priceRange = null;
		if (newMode._internal_autoScale !== void 0) this._private__options.autoScale = newMode._internal_autoScale;
		if (newMode._internal_mode !== void 0) {
			this._private__options.mode = newMode._internal_mode;
			if (newMode._internal_mode === 2 || newMode._internal_mode === 3) this._private__options.autoScale = true;
			this._private__invalidatedForRange._internal_isValid = false;
		}
		if (oldMode._internal_mode === 1 && newMode._internal_mode !== oldMode._internal_mode) if (canConvertPriceRangeFromLog(this._private__priceRange, this._private__logFormula)) {
			priceRange = convertPriceRangeFromLog(this._private__priceRange, this._private__logFormula);
			if (priceRange !== null) this._internal_setPriceRange(priceRange);
		} else this._private__options.autoScale = true;
		...
		if (modeChanged && (oldMode._internal_mode === 2 || this._internal_isPercentage())) this._internal_updateFormatter();
		if (modeChanged && (oldMode._internal_mode === 3 || this._internal_isIndexedTo100())) this._internal_updateFormatter();
	...
	_internal_setHeight(value) {
		if (this._private__height === value) return;
		this._private__height = value;
		this._private__invalidateInternalHeightCache();
		this._private__marksCache = null;
	}
	_internal_internalHeight() {
		if (this._private__internalHeightCache) return this._private__internalHeightCache;
	...
	_internal_setPriceRange(newPriceRange, isForceSetValue) {
		...
		if (!isForceSetValue && !(oldPriceRange === null && newPriceRange !== null) && (oldPriceRange === null || oldPriceRange._internal_equals(newPriceRange))) return;
		this._private__marksCache = null;
	...
	_internal_priceToCoordinate(price, baseValue) {
		if (this._internal_isPercentage()) price = toPercent(price, baseValue);
		else if (this._internal_isIndexedTo100()) price = toIndexedTo100(price, baseValue);
	...
	_internal_pointsArrayToCoordinates(points, baseValue, visibleRange) {
		...
		for (let i = fromIndex; i < toIndex; i++) {
			...
			let logical = price;
			if (transformFn !== null) logical = transformFn(point._internal_price, baseValue);
	...
	_internal_logicalToPrice(logical, baseValue) {
		let value = logical;
		if (this._internal_isPercentage()) value = fromPercent(value, baseValue);
		else if (this._internal_isIndexedTo100()) value = fromIndexedTo100(value, baseValue);
		return value;
	}
	...
	_internal_orderedSources() {
		if (this._private__cachedOrderedSources) return this._private__cachedOrderedSources;
		let sources = [];
		for (let i = 0; i < this._private__dataSources.length; i++) {
			const ds = this._private__dataSources[i];
			if (ds._internal_zorder() === null) ds._internal_setZorder(i + 1);
			sources.push(ds);
		}
	...
	_internal_addDataSource(source) {
		if (this._private__dataSources.indexOf(source) !== -1) return;
	...
	_internal_removeDataSource(source) {
		...
		if (index === -1) throw new Error("source is not attached to scale");
		this._private__dataSources.splice(index, 1);
		if (this._private__dataSources.length === 0) {
			this._internal_setMode({ _internal_autoScale: true });
	...
	_internal_firstValue() {
		...
		for (const source of this._private__dataSources) {
			...
			if (result === null || firstValue._internal_timePoint < result._internal_timePoint) result = firstValue;
		}
	...
	_internal_marks() {
		...
		if (this._private__marksCache !== null && (firstValueIsNull || this._private__marksCache._internal_firstValueIsNull === firstValueIsNull)) return this._private__marksCache._internal_marks;
	...
	_internal_startScale(x) {
		if (this._internal_isPercentage() || this._internal_isIndexedTo100()) return;
		if (this._private__scaleStartPoint !== null || this._private__priceRangeSnapshot !== null) return;
		if (this._internal_isEmpty()) return;
	...
	_internal_scaleTo(x) {
		if (this._internal_isPercentage() || this._internal_isIndexedTo100()) return;
		if (this._private__scaleStartPoint === null) return;
		this._internal_setMode({ _internal_autoScale: false });
		x = this._private__height - x;
		if (x < 0) x = 0;
		let scaleCoeff = (this._private__scaleStartPoint + (this._private__height - 1) * .2) / (x + (this._private__height - 1) * .2);
		const newPriceRange = ensureNotNull(this._private__priceRangeSnapshot)._internal_clone();
		scaleCoeff = Math.max(scaleCoeff, .1);
	...
	_internal_startScroll(x) {
		if (this._internal_isAutoScale()) return;
		if (this._private__scrollStartPoint !== null || this._private__priceRangeSnapshot !== null) return;
		if (this._internal_isEmpty()) return;
	...
	_internal_scrollTo(x) {
		if (this._internal_isAutoScale()) return;
		if (this._private__scrollStartPoint === null) return;
		...
		if (this._internal_isInverted()) pixelDelta *= -1;
	...
	_internal_formatter() {
		if (!this._private__formatter) this._internal_updateFormatter();
		return this._private__formatter;
	}
	_internal_formatPrice(price, firstValue) {
		switch (this._private__options.mode) {
			case 2: return this._private__formatPercentage(toPercent(price, firstValue));
			case 3: return this._internal_formatter().format(toIndexedTo100(price, firstValue));
			default: return this._private__formatPrice(price);
		}
	}
	_internal_formatLogical(logical) {
		switch (this._private__options.mode) {
			case 2: return this._private__formatPercentage(logical);
			case 3: return this._internal_formatter().format(logical);
			default: return this._private__formatPrice(logical);
		}
	}
	...
	_internal_updateFormatter() {
		...
		let base = 100;
		if (formatterSource !== null) base = Math.round(1 / formatterSource._internal_minMove());
		this._private__formatter = defaultPriceFormatter;
		if (this._internal_isPercentage()) {
			...
			base = 100;
		} else if (formatterSource !== null) this._private__formatter = formatterSource._internal_formatter();
	...
	*/
	...
	_private__logicalToCoordinate(logical, baseValue) {
		this._private__makeSureItIsValid();
		if (this._internal_isEmpty()) return 0;
	...
	_private__recalculatePriceRangeImpl() {
		...
		for (const source of sources) {
			if (!source._internal_visible()) continue;
			const firstValue = source._internal_firstValue();
			if (firstValue === null) continue;
			...
			if (sourceRange !== null) {
				...
				if (priceRange === null) priceRange = sourceRange;
				else priceRange = priceRange._internal_merge(ensureNotNull(sourceRange));
		...
		if (priceRange !== null) {
			if (priceRange._internal_minValue() === priceRange._internal_maxValue()) {
				const formatterSource = this._private__formatterSource();
				const extendValue = 5 * (formatterSource === null || this._internal_isPercentage() || this._internal_isIndexedTo100() ? 1 : formatterSource._internal_minMove());
				if (this._internal_isLog()) priceRange = convertPriceRangeFromLog(priceRange, this._private__logFormula);
				priceRange = new PriceRangeImpl(priceRange._internal_minValue() - extendValue, priceRange._internal_maxValue() + extendValue);
				if (this._internal_isLog()) priceRange = convertPriceRangeToLog(priceRange, this._private__logFormula);
			}
			if (this._internal_isLog()) {
				...
				if (!logFormulasAreSame(newLogFormula, this._private__logFormula)) {
					...
					if (rawSnapshot !== null) this._private__priceRangeSnapshot = convertPriceRangeToLog(rawSnapshot, newLogFormula);
				}
			}
			this._internal_setPriceRange(priceRange);
		} else if (this._private__priceRange === null) {
			this._internal_setPriceRange(new PriceRangeImpl(-.5, .5));
			this._private__logFormula = logFormulaForPriceRange(null);
		}
		this._private__invalidatedForRange._internal_isValid = true;
	}
	_private__getCoordinateTransformer() {
		if (this._internal_isPercentage()) return toPercent;
		else if (this._internal_isIndexedTo100()) return toIndexedTo100;
		else if (this._internal_isLog()) return (price) => toLog(price, this._private__logFormula);
		return null;
	}
	_private__formatValue(value, formatter, fallbackFormatter) {
		if (formatter === void 0) {
			if (fallbackFormatter === void 0) fallbackFormatter = this._internal_formatter();
...
var Pane = class {
	...
	_internal_applyScaleOptions(options) {
		if (options.leftPriceScale) this._private__leftPriceScale._internal_applyOptions(options.leftPriceScale);
		if (options.rightPriceScale) this._private__rightPriceScale._internal_applyOptions(options.rightPriceScale);
		...
		if (options.overlayPriceScales) {
			...
			for (const arr of sourceArrays) {
				...
				if (options.localization) priceScale._internal_updateFormatter();
			}
		}
	}
	_internal_priceScaleById(id) {
		switch (id) {
			case "left": return this._private__leftPriceScale;
			case "right": return this._private__rightPriceScale;
		}
		if (this._private__overlaySourcesByScaleId.has(id)) return ensureDefined(this._private__overlaySourcesByScaleId.get(id))[0]._internal_priceScale();
		return null;
	}
	_internal_destroy() {
		...
		this._private__dataSources.forEach((source) => {
			if (source._internal_destroy) source._internal_destroy();
		});
	...
	_internal_setHeight(height) {
		...
		this._private__dataSources.forEach((ds) => {
			if (this._internal_isOverlay(ds)) {
				...
				if (priceScale !== null) priceScale._internal_setHeight(height);
			}
		});
		this._internal_updateAllSources();
	}
	...
	_internal_isOverlay(source) {
		...
		if (priceScale === null) return true;
	...
	_internal_removeDataSource(source) {
		...
		if (this._private__overlaySourcesByScaleId.has(priceScaleId)) {
			...
			if (overlayIndex !== -1) {
				...
				if (overlaySources.length === 0) this._private__overlaySourcesByScaleId.delete(priceScaleId);
			}
		}
		const priceScale = source._internal_priceScale();
		if (priceScale && priceScale._internal_dataSources().indexOf(source) >= 0) priceScale._internal_removeDataSource(source);
		if (priceScale !== null) {
	...
	_internal_priceScalePosition(priceScale) {
		if (priceScale === this._private__leftPriceScale) return "left";
		if (priceScale === this._private__rightPriceScale) return "right";
		return "overlay";
	}
	...
	_internal_defaultPriceScale() {
		let priceScale = null;
		if (this._private__model._internal_options().rightPriceScale.visible && this._private__rightPriceScale._internal_dataSources().length !== 0) priceScale = this._private__rightPriceScale;
		else if (this._private__model._internal_options().leftPriceScale.visible && this._private__leftPriceScale._internal_dataSources().length !== 0) priceScale = this._private__leftPriceScale;
		else if (this._private__dataSources.length !== 0) priceScale = this._private__dataSources[0]._internal_priceScale();
		if (priceScale === null) priceScale = this._private__rightPriceScale;
		return priceScale;
	}
	_internal_defaultVisiblePriceScale() {
		let priceScale = null;
		if (this._private__model._internal_options().rightPriceScale.visible) priceScale = this._private__rightPriceScale;
		else if (this._private__model._internal_options().leftPriceScale.visible) priceScale = this._private__leftPriceScale;
		return priceScale;
	}
	_internal_recalculatePriceScale(priceScale) {
		if (priceScale === null || !priceScale._internal_isAutoScale()) return;
	...
	_internal_resetPriceScale(priceScale) {
		...
		if (visibleBars !== null) priceScale._internal_recalculatePriceRange(visibleBars);
		this._internal_updateAllSources();
	}
	...
	_internal_recalculate() {
		...
		this._private__dataSources.forEach((ds) => {
			if (this._internal_isOverlay(ds)) this._internal_recalculatePriceScale(ds._internal_priceScale());
		});
		this._internal_updateAllSources();
		this._private__model._internal_lightUpdate();
	}
	_internal_orderedSources() {
		if (this._private__cachedOrderedSources === null) this._private__cachedOrderedSources = sortSources(this._private__dataSources);
		return this._private__cachedOrderedSources;
	}
	...
	_private__getZOrderMinMax() {
		...
		for (let j = 0; j < sources.length; j++) {
			const zOrder = sources[j]._internal_zorder();
			if (zOrder !== null) {
	...
	_private__insertDataSource(source, priceScaleId, zOrder) {
		...
		if (priceScale === null) priceScale = this._private__createPriceScale(priceScaleId, this._private__model._internal_options().overlayPriceScales);
		this._private__dataSources.push(source);
	...
	_private__onPriceScaleModeChanged(priceScale, oldMode, newMode) {
		if (oldMode._internal_mode === newMode._internal_mode) return;
		this._private__recalculatePriceScaleImpl(priceScale);
	}
	_private__createPriceScale(id, options) {
		const priceScale = new PriceScale(id, Object.assign({
			visible: true,
			autoScale: true
		}, clone(options)), this._private__model._internal_options().layout, this._private__model._internal_options().localization);
...
var FormattedLabelsCache = class {
	...
	_internal_format(tickMark) {
		...
		if (tick !== void 0) return tick._internal_string;
		if (this._private__actualSize === this._private__maxSize) {
...
var TickMarks = class {
	...
	_internal_build(spacing, maxWidth) {
		...
		if (this._private__cache === null || this._private__cache._internal_maxIndexesPerMark !== maxIndexesPerMark) this._private__cache = {
	...
	_private__removeMarksSinceIndex(sinceIndex) {
		...
		this._private__marksByWeight.forEach((marks, timeWeight) => {
			if (sinceIndex <= marks[0].index) weightsToClear.push(timeWeight);
			else marks.splice(lowerBound(marks, sinceIndex, (tm) => tm.index < sinceIndex), Infinity);
		});
		for (const weight of weightsToClear) this._private__marksByWeight.delete(weight);
	}
	_private__buildMarksImpl(maxIndexesPerMark) {
		let marks = [];
		for (const weight of Array.from(this._private__marksByWeight.keys()).sort((a, b) => b - a)) {
			if (!this._private__marksByWeight.get(weight)) continue;
			...
			for (let i = 0; i < currentWeightLength; i++) {
				...
				} else if (this._private__uniformDistribution) return prevMarks;
			}
			for (; prevMarksPointer < prevMarksLength; prevMarksPointer++) marks.push(prevMarks[prevMarksPointer]);
		}
		return marks;
	}
};
var TimeScaleVisibleRange = class TimeScaleVisibleRange {
	constructor(logicalRange) {
		this._private__logicalRange = logicalRange;
	}
	_internal_strictRange() {
		if (this._private__logicalRange === null) return null;
...
var TimeScale = class {
	...
	_internal_applyOptions(options, localizationOptions) {
		var _a;
		merge(this._private__options, options);
		if (this._private__options.fixLeftEdge) this._private__doFixLeftEdge();
		if (this._private__options.fixRightEdge) this._private__doFixRightEdge();
		if (options.barSpacing !== void 0) this._private__model._internal_setBarSpacing(options.barSpacing);
		if (options.rightOffset !== void 0) this._private__model._internal_setRightOffset(options.rightOffset);
		if (options.minBarSpacing !== void 0) this._private__model._internal_setBarSpacing((_a = options.barSpacing) !== null && _a !== void 0 ? _a : this._private__barSpacing);
		this._private__invalidateTickMarks();
		this._private__updateDateTimeFormatter();
	...
	_internal_timeToIndex(time, findNearest) {
		if (this._private__points.length < 1) return null;
		if (this._private__horzScaleBehavior.key(time) > this._private__horzScaleBehavior.key(this._private__points[this._private__points.length - 1].time)) return findNearest ? this._private__points.length - 1 : null;
		...
		if (this._private__horzScaleBehavior.key(time) < this._private__horzScaleBehavior.key(this._private__points[index].time)) return findNearest ? index : null;
		return index;
	}
	_internal_isEmpty() {
	...
	_internal_setWidth(newWidth) {
		...
		if (this._private__width === newWidth) return;
		...
		this._private__visibleRangeInvalidated = true;
		if (this._private__options.lockVisibleTimeRangeOnResize && oldWidth !== 0) this._private__barSpacing = this._private__barSpacing * newWidth / oldWidth;
	...
	_internal_indexToCoordinate(index) {
		...
		const deltaFromRight = this._internal_baseIndex() + this._private__rightOffset - index;
		return this._private__width - (deltaFromRight + .5) * this._private__barSpacing - 1;
	}
	_internal_indexesToCoordinates(points, visibleRange) {
		...
		for (let i = indexFrom; i < indexTo; i++) {
			...
			const coordinate = this._private__width - (deltaFromRight + .5) * this._private__barSpacing - 1;
	...
	_internal_marks() {
		if (this._internal_isEmpty()) return null;
		if (this._private__timeMarksCache !== null) return this._private__timeMarksCache;
		const spacing = this._private__barSpacing;
		const maxLabelWidth = (this._private__model._internal_options().layout.fontSize + 4) * 5 / defaultTickMarkMaxCharacterLength * (this._private__options.tickMarkMaxCharacterLength || defaultTickMarkMaxCharacterLength);
		...
		for (const tm of items) {
			if (!(firstBar <= tm.index && tm.index <= lastBar)) continue;
			let label;
			...
			if (this._private__barSpacing > maxLabelWidth / 2 && !isAllScalingAndScrollingDisabled) label.needAlignCoordinate = false;
			else label.needAlignCoordinate = isLeftEdgeFixed && tm.index <= earliestIndexOfSecondLabel || isRightEdgeFixed && tm.index >= indexOfSecondLastLabel;
			targetIndex++;
		}
	...
	*/
	_internal_zoom(zoomPoint, scale) {
		...
		if (!this._private__options.rightBarStaysOnScroll) this._internal_setRightOffset(this._internal_rightOffset() + (floatIndexAtZoomPoint - this._private__coordinateToFloatIndex(zoomPoint)));
	}
	_internal_startScale(x) {
		if (this._private__scrollStartPoint) this._internal_endScroll();
		if (this._private__scaleStartPoint !== null || this._private__commonTransitionStartState !== null) return;
		if (this._internal_isEmpty()) return;
		this._private__scaleStartPoint = x;
		this._private__saveCommonTransitionsStartState();
	}
	_internal_scaleTo(x) {
		if (this._private__commonTransitionStartState === null) return;
		...
		if (startLengthFromRight === 0 || currentLengthFromRight === 0) return;
	...
	_internal_startScroll(x) {
		if (this._private__scrollStartPoint !== null || this._private__commonTransitionStartState !== null) return;
		if (this._internal_isEmpty()) return;
		this._private__scrollStartPoint = x;
		this._private__saveCommonTransitionsStartState();
	}
	...
	_internal_scrollToOffsetAnimated(offset, animationDuration = 400) {
		if (!isFinite(offset)) throw new RangeError("offset is required and must be finite number");
		if (!isFinite(animationDuration) || animationDuration <= 0) throw new RangeError("animationDuration (optional) must be finite positive number");
		...
		this._private__model._internal_setTimeScaleAnimation({
			...
			_internal_getPosition: (time) => {
				...
				return animationProgress >= 1 ? offset : source + (offset - source) * animationProgress;
			}
		});
	}
	...
	_internal_formatDateTime(timeScalePoint) {
		if (this._private__localizationOptions.timeFormatter !== void 0) return this._private__localizationOptions.timeFormatter(timeScalePoint.originalTime);
	...
	_private__coordinateToFloatIndex(x) {
		...
		const index = this._internal_baseIndex() + this._private__rightOffset - deltaFromRight;
	...
	_private__updateVisibleRange() {
		if (!this._private__visibleRangeInvalidated) return;
		...
		const logicalRange = new RangeImpl(rightBorder - newBarsLength + 1, rightBorder);
	...
	_private__correctBarSpacing() {
		...
		if (this._private__width !== 0) {
			const maxBarSpacing = this._private__width * .5;
	...
	_private__minBarSpacing() {
		if (this._private__options.fixLeftEdge && this._private__options.fixRightEdge && this._private__points.length !== 0) return this._private__width / this._private__points.length;
	...
	_private__minRightOffset() {
		...
		if (firstIndex === null || baseIndex === null) return null;
	...
	_private__setVisibleRange(newVisibleRange) {
		...
		if (!areRangesEqual(oldVisibleRange._internal_strictRange(), this._private__visibleRange._internal_strictRange())) this._private__visibleBarsChanged._internal_fire();
		if (!areRangesEqual(oldVisibleRange._internal_logicalRange(), this._private__visibleRange._internal_logicalRange())) this._private__logicalRangeChanged._internal_fire();
		this._private__resetTimeMarksCache();
	}
	...
	_private__doFixLeftEdge() {
		if (!this._private__options.fixLeftEdge) return;
		const firstIndex = this._private__firstIndex();
...
var WatermarkRenderer = class extends MediaCoordinatesPaneRenderer {
	...
	_internal__drawBackgroundImpl(renderingScope) {
		if (!this._private__data._internal_visible) return;
		const { context: ctx, mediaSize } = renderingScope;
		let textHeight = 0;
		for (const line of this._private__data._internal_lines) {
			if (line._internal_text.length === 0) continue;
			ctx.font = line._internal_font;
			const textWidth = this._private__metrics(ctx, line._internal_text);
			if (textWidth > mediaSize.width) line._internal_zoom = mediaSize.width / textWidth;
			else line._internal_zoom = 1;
...
var WatermarkPaneView = class {
	...
	_private__updateImpl() {
		...
		if (!data._internal_visible) return;
...
var ChartModel = class {
	constructor(invalidateHandler, options, horzScaleBehavior) {
		...
		this._private__backgroundTopColor = this._private__getBackgroundColor(0);
		this._private__backgroundBottomColor = this._private__getBackgroundColor(1);
	}
	...
	_internal_cursorUpdate() {
		this._private__invalidate(new InvalidateMask(1));
	}
	...
	_internal_setHoveredSource(source) {
		...
		if (prevSource !== null) this._internal_updateSource(prevSource._internal_source);
		if (source !== null) this._internal_updateSource(source._internal_source);
	}
	_internal_options() {
		return this._private__options;
	}
	_internal_applyOptions(options) {
		...
		if (options.timeScale !== void 0) this._private__timeScale._internal_applyOptions(options.timeScale);
		if (options.localization !== void 0) this._private__timeScale._internal_applyLocalizationOptions(options.localization);
		if (options.leftPriceScale || options.rightPriceScale) this._private__priceScalesOptionsChanged._internal_fire();
		this._private__backgroundTopColor = this._private__getBackgroundColor(0);
		this._private__backgroundBottomColor = this._private__getBackgroundColor(1);
	...
	_internal_applyPriceScaleOptions(priceScaleId, options) {
		if (priceScaleId === "left") {
			this._internal_applyOptions({ leftPriceScale: options });
			return;
		} else if (priceScaleId === "right") {
			this._internal_applyOptions({ rightPriceScale: options });
			return;
		}
		const res = this._internal_findPriceScale(priceScaleId);
		if (res === null) throw new Error(`Trying to apply price scale options with incorrect ID: ${priceScaleId}`);
	...
	_internal_createPane(index) {
		...
		if (index !== void 0) this._private__panes.splice(index, 0, pane);
		else this._private__panes.push(pane);
	...
	_internal_scalePriceTo(pane, priceScale, x) {
		...
		this._private__invalidate(this._private__paneInvalidationMask(pane, 2));
	}
	...
	_internal_startScrollPrice(pane, priceScale, x) {
		if (priceScale._internal_isAutoScale()) return;
	...
	*/
	_internal_zoomTime(pointX, scale) {
		...
		if (timeScale._internal_isEmpty() || scale === 0) return;
	...
	_internal_setAndSaveCurrentPosition(x, y, event, pane, skipEvent) {
		...
		const visibleBars = this._private__timeScale._internal_visibleStrictRange();
		if (visibleBars !== null) index = Math.min(Math.max(visibleBars._internal_left(), index), visibleBars._internal_right());
		const priceScale = pane._internal_defaultPriceScale();
		const firstValue = priceScale._internal_firstValue();
		if (firstValue !== null) price = priceScale._internal_coordinateToPrice(y, firstValue);
		...
		if (!skipEvent) this._private__crosshairMoved._internal_fire(this._private__crosshair._internal_appliedIndex(), {
			x,
			y
		}, event);
	}
	...
	_internal_clearCurrentPosition(skipEvent) {
		this._internal_crosshairSource()._internal_clearPosition();
		this._internal_cursorUpdate();
		if (!skipEvent) this._private__crosshairMoved._internal_fire(null, null, null);
	}
	...
	_internal_updateTimeScale(newBaseIndex, newPoints, firstChangedPointIndex) {
		...
		if (newPoints !== void 0 && firstChangedPointIndex !== void 0) this._private__timeScale._internal_update(newPoints, firstChangedPointIndex);
		...
		if (visibleBars !== null && oldFirstTime !== null && newFirstTime !== null) {
			...
			const isSeriesPointsAddedToRight = newBaseIndex !== null && newBaseIndex > currentBaseIndex && !isLeftBarShiftToLeft;
			...
			const needShiftVisibleRangeOnNewBar = isLastSeriesBarVisible && (!(firstChangedPointIndex === void 0) || allowShiftWhenReplacingWhitespace) && this._private__timeScale._internal_options().shiftVisibleRangeOnNewBar;
	...
	_internal_recalculatePane(pane) {
		if (pane !== null) pane._internal_recalculate();
	}
	...
	_internal_createSeries(seriesType, options, customPaneView) {
		...
		if (this._private__serieses.length === 1) this._internal_fullUpdate();
		else this._internal_lightUpdate();
		return series;
	}
	_internal_removeSeries(series) {
		...
		if (series._internal_destroy) series._internal_destroy();
	}
	...
	_internal_backgroundColorAtYPercentFromTop(percent) {
		...
		if (bottomColor === topColor) return bottomColor;
		percent = Math.max(0, Math.min(100, Math.round(percent * 100)));
		if (this._private__gradientColorsCache === null || this._private__gradientColorsCache._internal_topColor !== topColor || this._private__gradientColorsCache._internal_bottomColor !== bottomColor) this._private__gradientColorsCache = {
	...
	_private__createSeries(options, seriesType, pane, customPaneView) {
		...
		if (!isDefaultPriceScale(targetScaleId)) series._internal_applyOptions(options);
		return series;
	}
	_private__getBackgroundColor(side) {
		const layoutOptions = this._private__options.layout;
		if (layoutOptions.background.type === "gradient") return side === 0 ? layoutOptions.background.topColor : layoutOptions.background.bottomColor;
...
function defaultTickMarkFormatter(timePoint, tickMarkType, locale) {
	...
	const date = timePoint._internal_businessDay === void 0 ? /* @__PURE__ */ new Date(timePoint._internal_timestamp * 1e3) : new Date(Date.UTC(timePoint._internal_businessDay.year, timePoint._internal_businessDay.month - 1, timePoint._internal_businessDay.day));
	return new Date(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate(), date.getUTCHours(), date.getUTCMinutes(), date.getUTCSeconds(), date.getUTCMilliseconds()).toLocaleString(locale, formatOptions);
}
...
function weightByTime(currentDate, prevDate) {
	if (currentDate.getUTCFullYear() !== prevDate.getUTCFullYear()) return 70;
	else if (currentDate.getUTCMonth() !== prevDate.getUTCMonth()) return 60;
	else if (currentDate.getUTCDate() !== prevDate.getUTCDate()) return 50;
	for (let i = intradayWeightDivisors.length - 1; i >= 0; --i) if (Math.floor(prevDate.getTime() / intradayWeightDivisors[i]._internal_divisor) !== Math.floor(currentDate.getTime() / intradayWeightDivisors[i]._internal_divisor)) return intradayWeightDivisors[i]._internal_weight;
	return 0;
}
function cast(t) {
	return t;
...
function fillWeightsForPoints(sortedTimePoints, startIndex = 0) {
	...
	let prevDate = prevTime !== null ? /* @__PURE__ */ new Date(prevTime * 1e3) : null;
	let totalTimeDiff = 0;
	for (let index = startIndex; index < sortedTimePoints.length; ++index) {
		const currentPoint = sortedTimePoints[index];
		const currentDate = /* @__PURE__ */ new Date(cast(currentPoint.time)._internal_timestamp * 1e3);
		if (prevDate !== null) currentPoint.timeWeight = weightByTime(currentDate, prevDate);
	...
	if (startIndex === 0 && sortedTimePoints.length > 1) {
		...
		const approxPrevDate = /* @__PURE__ */ new Date((cast(sortedTimePoints[0].time)._internal_timestamp - averageTimeDiff) * 1e3);
		sortedTimePoints[0].timeWeight = weightByTime(/* @__PURE__ */ new Date(cast(sortedTimePoints[0].time)._internal_timestamp * 1e3), approxPrevDate);
	}
}
function businessDayConverter(time) {
	let businessDay = time;
	if (isString(time)) businessDay = stringToBusinessDay(time);
	if (!isBusinessDay(businessDay)) throw new Error("time must be of type BusinessDay");
...
function timestampConverter(time) {
	if (!isUTCTimestamp(time)) throw new Error("time must be of type isUTCTimestamp");
	return { _internal_timestamp: time };
}
function selectTimeConverter(data) {
	if (data.length === 0) return null;
	if (isBusinessDay(data[0].time) || isString(data[0].time)) return businessDayConverter;
	return timestampConverter;
}
var validDateRegex = /^\d\d\d\d-\d\d-\d\d$/;
function convertTime(time) {
	if (isUTCTimestamp(time)) return timestampConverter(time);
	if (!isBusinessDay(time)) return businessDayConverter(stringToBusinessDay(time));
	return businessDayConverter(time);
}
function stringToBusinessDay(value) {
	if (!validDateRegex.test(value)) throw new Error(`Invalid date string=${value}, expected format=yyyy-mm-dd`);
	const d = new Date(value);
	if (isNaN(d.getTime())) throw new Error(`Invalid date string=${value}, expected format=yyyy-mm-dd`);
	return {
		day: d.getUTCDate(),
...
function weightToTickMarkType(weight, timeVisible, secondsVisible) {
	switch (weight) {
		case 0:
		case 10: return timeVisible ? secondsVisible ? 4 : 3 : 2;
		case 20:
		case 21:
		case 22:
		case 30:
		case 31:
		case 32:
		case 33: return timeVisible ? 3 : 2;
		case 50: return 2;
		case 60: return 1;
		case 70: return 0;
	}
...
var HorzScaleBehaviorTime = class {
	...
	preprocessData(data) {
		if (Array.isArray(data)) convertStringsToBusinessDays(data);
		else convertStringToBusinessDay(data);
	}
	createConverterToInternalObj(data) {
		return ensureNotNull(selectTimeConverter(data));
	}
	key(item) {
		if (typeof item === "object" && "_internal_timestamp" in item) return item._internal_timestamp;
		else return this.key(this.convertHorzItemToInternal(item));
	}
	cacheKey(item) {
		const time = item;
		return time._internal_businessDay === void 0 ? (/* @__PURE__ */ new Date(time._internal_timestamp * 1e3)).getTime() : new Date(Date.UTC(time._internal_businessDay.year, time._internal_businessDay.month - 1, time._internal_businessDay.day)).getTime();
	}
	convertHorzItemToInternal(item) {
		return convertTime(item);
	}
	updateFormatter(options) {
		if (!this._private__options) return;
		const dateFormat = options.dateFormat;
		if (this._private__options.timeScale.timeVisible) this._private__dateTimeFormatter = new DateTimeFormatter({
		...
		else this._private__dateTimeFormatter = new DateFormatter(dateFormat, options.locale);
	}
	formatHorzItem(item) {
		const tp = item;
		return this._private__dateTimeFormatter._internal_format(/* @__PURE__ */ new Date(tp._internal_timestamp * 1e3));
	}
	formatTickmark(tickMark, localizationOptions) {
		...
		if (options.tickMarkFormatter !== void 0) {
			...
			if (tickMarkString !== null) return tickMarkString;
		}
	...
	maxTickMarkWeight(tickMarks) {
		...
		if (maxWeight > 30 && maxWeight < 50) maxWeight = 30;
		return maxWeight;
	}
...
function isFF() {
	if (!isRunningOnClientSide) return false;
...
function isWindows() {
	var _a;
	if (!isRunningOnClientSide) return false;
	if ((_a = navigator === null || navigator === void 0 ? void 0 : navigator.userAgentData) === null || _a === void 0 ? void 0 : _a.platform) return navigator.userAgentData.platform === "Windows";
...
function suggestChartSize(originalSize) {
	...
	return size$1({
		width: integerWidth - integerWidth % 2,
		height: integerHeight - integerHeight % 2
	});
}
...
var KineticAnimation = class {
	...
	_internal_addPosition(position, time) {
		if (this._private__position1 !== null) {
			...
			if (Math.abs(this._private__position1._internal_position - position) < this._private__minMove) return;
		}
	...
	_internal_start(position, time) {
		if (this._private__position1 === null || this._private__position2 === null) return;
		if (time - this._private__position1._internal_time > 50) return;
		let totalDistance = 0;
		...
		let resultSpeed = 0;
		for (let i = 0; i < speedItems.length; ++i) resultSpeed += distanceItems[i] / totalDistance * speedItems[i];
		if (Math.abs(resultSpeed) < this._private__minSpeed) return;
	...
	_private__progressDuration(time) {
		const progress = time - ensureNotNull(this._private__animationStartPosition)._internal_time;
...
var AttributionLogoWidget = class {
	...
	_internal_removeElement() {
		if (this._private__element) this._private__container.removeChild(this._private__element);
		if (this._private__cssElement) this._private__container.removeChild(this._private__cssElement);
		this._private__element = void 0;
		this._private__cssElement = void 0;
	}
	...
	_private__render() {
		if (!this._private__shouldUpdate()) return;
...
function createBoundCanvas(parentElement, size) {
	const canvas = ensureNotNull(parentElement.ownerDocument).createElement("canvas");
...
function releaseCanvas(canvas) {
	...
	canvas.width = 1;
	canvas.height = 1;
	(_a = canvas.getContext("2d")) === null || _a === void 0 || _a.clearRect(0, 0, 1, 1);
}
function drawBackground(renderer, target, isHovered, hitTestData) {
	if (renderer._internal_drawBackground) renderer._internal_drawBackground(target, isHovered, hitTestData);
}
...
function drawSourcePaneViews(paneViewsGetter, drawRendererFn, source, pane) {
	...
	for (const paneView of paneViews) {
		...
		if (renderer !== null) drawRendererFn(renderer);
	}
}
...
var MouseEventHandler = class {
		...
		this._private__onMobileSafariDoubleClick = (dblClickEvent) => {
			if (this._private__firesTouchEvents(dblClickEvent)) {
				...
				if (this._private__tapTimeoutId && this._private__tapCount > 1) {
					...
					if (manhattanDistance < 30 && !this._private__cancelTap) this._private__processTouchEvent(compatEvent, this._private__handler._internal_doubleTapEvent);
				...
				if (this._private__clickTimeoutId && this._private__clickCount > 1) {
					...
					if (manhattanDistance < 5 && !this._private__cancelClick) this._private__processMouseEvent(compatEvent, this._private__handler._internal_mouseDoubleClickEvent);
	...
	_private__mouseEnterHandler(enterEvent) {
		if (this._private__unsubscribeMousemove) this._private__unsubscribeMousemove();
		...
		if (this._private__firesTouchEvents(enterEvent)) return;
	...
	_private__resetClickTimeout() {
		if (this._private__clickTimeoutId !== null) clearTimeout(this._private__clickTimeoutId);
		this._private__clickCount = 0;
		this._private__clickTimeoutId = null;
		this._private__clickPosition = {
			_internal_x: Number.NEGATIVE_INFINITY,
	...
	_private__resetTapTimeout() {
		if (this._private__tapTimeoutId !== null) clearTimeout(this._private__tapTimeoutId);
		this._private__tapCount = 0;
		this._private__tapTimeoutId = null;
		this._private__tapPosition = {
			_internal_x: Number.NEGATIVE_INFINITY,
	...
	_private__mouseMoveHandler(moveEvent) {
		if (this._private__mousePressed || this._private__touchMoveStartPosition !== null) return;
		if (this._private__firesTouchEvents(moveEvent)) return;
	...
	_private__touchMoveHandler(moveEvent) {
		...
		if (this._private__startPinchMiddlePoint !== null) return;
		if (this._private__preventTouchDragProcess) return;
		this._private__pinchPrevented = true;
		const { _internal_xOffset: xOffset, _internal_yOffset: yOffset, _internal_manhattanDistance: manhattanDistance } = this._private__touchMouseMoveWithDownInfo(getPosition(touch), ensureNotNull(this._private__touchMoveStartPosition));
		if (!this._private__touchMoveExceededManhattanDistance && manhattanDistance < 5) return;
		if (!this._private__touchMoveExceededManhattanDistance) {
			...
			if (!isVertDrag && !isHorzDrag) this._private__preventTouchDragProcess = true;
	...
	_private__mouseMoveWithDownHandler(moveEvent) {
		if (moveEvent.button !== 0) return;
		const { _internal_manhattanDistance: manhattanDistance } = this._private__touchMouseMoveWithDownInfo(getPosition(moveEvent), ensureNotNull(this._private__mouseMoveStartPosition));
	...
	_private__touchMouseMoveWithDownInfo(currentPosition, startPosition) {
		...
		return {
			...
			_internal_manhattanDistance: xOffset + yOffset
		};
	}
	_private__touchEndHandler(touchEndEvent) {
		...
		if (this._private__tapTimeoutId && this._private__tapCount > 1) {
			...
			if (manhattanDistance < 30 && !this._private__cancelTap) this._private__processTouchEvent(compatEvent, this._private__handler._internal_doubleTapEvent);
			this._private__resetTapTimeout();
		} else if (!this._private__cancelTap) {
			this._private__processTouchEvent(compatEvent, this._private__handler._internal_tapEvent);
			if (this._private__handler._internal_tapEvent) preventDefault(touchEndEvent);
		}
		if (this._private__tapCount === 0) preventDefault(touchEndEvent);
	...
	_private__mouseUpHandler(mouseUpEvent) {
		...
		if (isFF()) this._private__target.ownerDocument.documentElement.removeEventListener("mouseleave", this._private__onFirefoxOutsideMouseUp);
		if (this._private__firesTouchEvents(mouseUpEvent)) return;
		...
		if (this._private__clickTimeoutId && this._private__clickCount > 1) {
			...
			if (manhattanDistance < 5 && !this._private__cancelClick) this._private__processMouseEvent(compatEvent, this._private__handler._internal_mouseDoubleClickEvent);
			this._private__resetClickTimeout();
		} else if (!this._private__cancelClick) this._private__processMouseEvent(compatEvent, this._private__handler._internal_mouseClickEvent);
	}
	_private__clearLongTapTimeout() {
		if (this._private__longTapTimeoutId === null) return;
		clearTimeout(this._private__longTapTimeoutId);
		this._private__longTapTimeoutId = null;
	}
	_private__touchStartHandler(downEvent) {
		if (this._private__activeTouchId !== null) return;
		...
		}
		{
			...
			this._private__clearLongTapTimeout();
			this._private__longTapTimeoutId = setTimeout(this._private__longTapHandler.bind(this, downEvent), 240);
		}
		...
		if (!this._private__tapTimeoutId) {
			this._private__tapCount = 0;
			this._private__tapTimeoutId = setTimeout(this._private__resetTapTimeout.bind(this), 500);
	...
	_private__mouseDownHandler(downEvent) {
		...
		const rootElement = this._private__target.ownerDocument.documentElement;
		if (isFF()) rootElement.addEventListener("mouseleave", this._private__onFirefoxOutsideMouseUp);
		this._private__cancelClick = false;
		...
		if (this._private__firesTouchEvents(downEvent)) return;
		...
		if (!this._private__clickTimeoutId) {
			this._private__clickCount = 0;
			this._private__clickTimeoutId = setTimeout(this._private__resetClickTimeout.bind(this), 500);
	...
	_private__init() {
		...
		this._private__target.addEventListener("touchcancel", this._private__clearLongTapTimeout.bind(this));
		{
			...
			const outsideHandler = (event) => {
				if (!this._private__handler._internal_mouseDownOutsideEvent) return;
				if (event.composed && this._private__target.contains(event.composedPath()[0])) return;
				if (event.target && this._private__target.contains(event.target)) return;
	...
	_private__initPinch() {
		if (this._private__handler._internal_pinchStartEvent === void 0 && this._private__handler._internal_pinchEvent === void 0 && this._private__handler._internal_pinchEndEvent === void 0) return;
		...
		this._private__target.addEventListener("touchmove", (event) => {
			if (event.touches.length !== 2 || this._private__startPinchMiddlePoint === null) return;
			if (this._private__handler._internal_pinchEvent !== void 0) {
				const scale = getDistance(event.touches[0], event.touches[1]) / this._private__startPinchDistance;
	...
	_private__checkPinchState(touches) {
		if (touches.length === 1) this._private__pinchPrevented = false;
		if (touches.length !== 2 || this._private__pinchPrevented || this._private__longTapActive) this._private__stopPinch();
		else this._private__startPinch(touches);
	}
	_private__startPinch(touches) {
		...
		if (this._private__handler._internal_pinchStartEvent !== void 0) this._private__handler._internal_pinchStartEvent();
		this._private__clearLongTapTimeout();
	}
	_private__stopPinch() {
		if (this._private__startPinchMiddlePoint === null) return;
		this._private__startPinchMiddlePoint = null;
		if (this._private__handler._internal_pinchEndEvent !== void 0) this._private__handler._internal_pinchEndEvent();
	}
	_private__mouseLeaveHandler(event) {
		if (this._private__unsubscribeMousemove) this._private__unsubscribeMousemove();
		if (this._private__firesTouchEvents(event)) return;
		if (!this._private__acceptMouseLeave) return;
	...
	_private__firesTouchEvents(e) {
		if (e.sourceCapabilities && e.sourceCapabilities.firesTouchEvents !== void 0) return e.sourceCapabilities.firesTouchEvents;
		return eventTimeStamp(e) < this._private__lastTouchEventTimeStamp + 500;
	}
	_private__processTouchEvent(event, callback) {
		if (callback) callback.call(this._private__handler, event);
	}
	...
	_private__makeCompatEvent(event, touch) {
		...
		return {
			...
			_internal_preventDefault: () => {
				if (event.type !== "touchstart") preventDefault(event);
			}
		};
	}
};
...
function findBestPrimitiveHitTest(sources, x, y) {
	...
	for (const source of sources) {
		...
		for (const hitResult of primitiveHitResults) if (comparePrimitiveZOrder(hitResult.zOrder, bestPrimitiveHit === null || bestPrimitiveHit === void 0 ? void 0 : bestPrimitiveHit.zOrder)) {
			bestPrimitiveHit = hitResult;
			bestHitSource = source;
		}
	}
	if (!bestPrimitiveHit || !bestHitSource) return null;
	return {
...
function convertPrimitiveHitResult(primitiveHit) {
	return {
		...
		_internal_object: { _internal_externalId: primitiveHit._internal_hit.externalId },
...
function hitTestPane(pane, x, y) {
	...
	if ((bestPrimitiveHit === null || bestPrimitiveHit === void 0 ? void 0 : bestPrimitiveHit._internal_hit.zOrder) === "top") return convertPrimitiveHitResult(bestPrimitiveHit);
	for (const source of sources) {
		if (bestPrimitiveHit && bestPrimitiveHit._internal_source === source && bestPrimitiveHit._internal_hit.zOrder !== "bottom" && !bestPrimitiveHit._internal_hit.isBackground) return convertPrimitiveHitResult(bestPrimitiveHit);
	...
	if (bestPrimitiveHit === null || bestPrimitiveHit === void 0 ? void 0 : bestPrimitiveHit._internal_hit) return convertPrimitiveHitResult(bestPrimitiveHit);
	return null;
}
function buildPriceAxisViewsGetter(zOrder, priceScaleId) {
	return (source) => {
		var _a, _b, _c, _d;
		if (((_b = (_a = source._internal_priceScale()) === null || _a === void 0 ? void 0 : _a._internal_id()) !== null && _b !== void 0 ? _b : "") !== priceScaleId) return [];
...
function recalculateOverlapping(views, direction, scaleHeight, rendererOptions) {
	...
	for (let i = 1; i < views.length; i++) {
		...
		if (direction === 1 ? coordinate > prevFixedCoordinate - height : coordinate < prevFixedCoordinate + height) {
			...
			if ((direction === 1 ? edgePoint < 0 : edgePoint > scaleHeight) && spaceBeforeCurrentGroup > 0) {
				...
				for (let k = currentGroupStart; k < views.length; k++) views[k]._internal_setFixedCoordinate(views[k]._internal_getFixedCoordinate() + direction * possibleShift);
...
var PriceAxisWidget = class {
	constructor(pane, options, rendererOptionsProvider, side) {
		...
		this._private__canvasSuggestedBitmapSizeChangedHandler = () => {
			if (this._private__isSettingSize) return;
	...
	_internal_destroy() {
		...
		if (this._private__priceScale !== null) this._private__priceScale._internal_onMarksChanged()._internal_unsubscribeAll(this);
		this._private__priceScale = null;
	}
	...
	_internal_rendererOptions() {
		...
		if (this._private__font !== options._internal_font) {
	...
	_internal_optimalWidth() {
		if (this._private__priceScale === null) return 0;
		...
		if (tickMarks.length > 0) tickMarkMaxWidth = Math.max(this._private__widthCache._internal_measureText(ctx, tickMarks[0]._internal_label), this._private__widthCache._internal_measureText(ctx, tickMarks[tickMarks.length - 1]._internal_label));
		...
		for (let j = views.length; j--;) {
			...
			if (width > tickMarkMaxWidth) tickMarkMaxWidth = width;
		}
		const firstValue = this._private__priceScale._internal_firstValue();
		if (firstValue !== null && this._private__size !== null && priceScaleCrosshairLabelVisible(this._private__options.crosshair)) {
			...
			tickMarkMaxWidth = Math.max(tickMarkMaxWidth, this._private__widthCache._internal_measureText(ctx, this._private__priceScale._internal_formatPrice(Math.floor(Math.min(topValue, bottomValue)) + .11111111111111, firstValue)), this._private__widthCache._internal_measureText(ctx, this._private__priceScale._internal_formatPrice(Math.ceil(Math.max(topValue, bottomValue)) - .11111111111111, firstValue)));
		}
		ctx.restore();
		const resultTickMarksMaxWidth = tickMarkMaxWidth || 34;
		return suggestPriceScaleWidth(Math.ceil(rendererOptions._internal_borderSize + rendererOptions._internal_tickLength + rendererOptions._internal_paddingInner + rendererOptions._internal_paddingOuter + 5 + resultTickMarksMaxWidth));
	}
	...
	_internal_setPriceScale(priceScale) {
		if (this._private__priceScale === priceScale) return;
		if (this._private__priceScale !== null) this._private__priceScale._internal_onMarksChanged()._internal_unsubscribeAll(this);
		this._private__priceScale = priceScale;
	...
	_internal_reset() {
		...
		this._private__pane._internal_chart()._internal_model()._internal_resetPriceScale(pane, ensureNotNull(this._internal_priceScale()));
	}
	_internal_paint(type) {
		if (this._private__size === null) return;
		if (type !== 1) {
	...
	_internal_drawBitmap(ctx, x, y) {
		...
		if (bitmapSize.width > 0 && bitmapSize.height > 0) ctx.drawImage(this._private__canvasBinding.canvasElement, x, y);
	}
	_internal_update() {
		var _a;
		(_a = this._private__priceScale) === null || _a === void 0 || _a._internal_marks();
	}
	_private__mouseDownEvent(e) {
		if (this._private__priceScale === null || this._private__priceScale._internal_isEmpty() || !this._private__options.handleScale.axisPressedMouseMove.price) return;
	...
	_private__pressedMouseMoveEvent(e) {
		if (this._private__priceScale === null || !this._private__options.handleScale.axisPressedMouseMove.price) return;
		const model = this._private__pane._internal_chart()._internal_model();
		const pane = this._private__pane._internal_state();
		const priceScale = this._private__priceScale;
	...
	_private__mouseDoubleClickEvent(e) {
		if (this._private__options.handleScale.axisDoubleClickReset.price) this._internal_reset();
	}
	_private__mouseEnterEvent(e) {
		if (this._private__priceScale === null) return;
		if (this._private__pane._internal_chart()._internal_model()._internal_options().handleScale.axisPressedMouseMove.price && !this._private__priceScale._internal_isPercentage() && !this._private__priceScale._internal_isIndexedTo100()) this._private__setCursor(1);
	}
	_private__mouseLeaveEvent(e) {
		this._private__setCursor(0);
	}
	_private__backLabels() {
		...
		const addViewsForSources = (sources) => {
			for (let i = 0; i < sources.length; ++i) {
				const views = sources[i]._internal_priceAxisViews(this._private__pane._internal_state(), priceScale);
				for (let j = 0; j < views.length; j++) res.push(views[j]);
			}
		};
	...
	_private__drawBackground({ context: ctx, bitmapSize }) {
		...
		if (topColor === bottomColor) clearRect(ctx, 0, 0, width, height, topColor);
		else clearRectWithGradient(ctx, 0, 0, width, height, topColor, bottomColor);
	}
	_private__drawBorder({ context: ctx, bitmapSize, horizontalPixelRatio }) {
		if (this._private__size === null || this._private__priceScale === null || !this._private__priceScale._internal_options().borderVisible) return;
		...
		let left;
		if (this._private__isLeft) left = bitmapSize.width - borderSize;
		else left = 0;
		ctx.fillRect(left, 0, borderSize, bitmapSize.height);
	}
	_private__drawTickMarks(target) {
		if (this._private__size === null || this._private__priceScale === null) return;
		const tickMarks = this._private__priceScale._internal_marks();
		...
		if (priceScaleOptions.borderVisible && priceScaleOptions.ticksVisible) target.useBitmapCoordinateSpace(({ context: ctx, horizontalPixelRatio, verticalPixelRatio }) => {
			ctx.fillStyle = priceScaleOptions.borderColor;
			const tickHeight = Math.max(1, Math.floor(verticalPixelRatio));
			const tickOffset = Math.floor(verticalPixelRatio * .5);
			const tickLength = Math.round(rendererOptions._internal_tickLength * horizontalPixelRatio);
			ctx.beginPath();
			for (const tickMark of tickMarks) ctx.rect(Math.floor(tickMarkLeftX * horizontalPixelRatio), Math.round(tickMark._internal_coord * verticalPixelRatio) - tickOffset, tickLength, tickHeight);
			ctx.fill();
		});
		target.useMediaCoordinateSpace(({ context: ctx }) => {
			var _a;
	...
	_private__alignLabels() {
		...
		const paneState = this._private__pane._internal_state();
		const rendererOptions = this._internal_rendererOptions();
		if (this._private__priceScale === paneState._internal_defaultVisiblePriceScale()) this._private__pane._internal_state()._internal_orderedSources().forEach((source) => {
			if (paneState._internal_isOverlay(source)) orderedSources.push(source);
		});
		const priceScale = this._private__priceScale;
		const updateForSources = (sources) => {
			sources.forEach((source) => {
				source._internal_priceAxisViews(paneState, priceScale).forEach((view) => {
					view._internal_setFixedCoordinate(null);
					if (view._internal_isVisible()) views.push(view);
				});
			});
		};
		updateForSources(orderedSources);
		views.forEach((view) => view._internal_setFixedCoordinate(view._internal_coordinate()));
		if (!this._private__priceScale._internal_options().alignLabels) return;
	...
	_private__fixLabelOverlap(views, rendererOptions) {
		...
		for (const view of views) {
			...
			if (coordinate > -halfHeight && coordinate < halfHeight) view._internal_setFixedCoordinate(halfHeight);
			if (coordinate > this._private__size.height - halfHeight && coordinate < this._private__size.height + halfHeight) view._internal_setFixedCoordinate(this._private__size.height - halfHeight);
		}
	...
	_private__drawBackLabels(target) {
		...
		views.forEach((view) => {
			if (view._internal_isAxisLabelVisible()) view._internal_renderer(ensureNotNull(this._private__priceScale))._internal_draw(target, rendererOptions, this._private__widthCache, align);
		});
	}
	...
	_private__onMarksChanged() {
		...
		if (this._private__prevOptimalWidth < width) this._private__pane._internal_chart()._internal_model()._internal_fullUpdate();
...
var PaneWidget = class PaneWidget {
	constructor(chart, state) {
		...
		this._private__isSettingSize = false;
		this._private__canvasSuggestedBitmapSizeChangedHandler = () => {
			if (this._private__isSettingSize || this._private__state === null) return;
			this._private__model()._internal_lightUpdate();
		};
		this._private__topCanvasSuggestedBitmapSizeChangedHandler = () => {
			if (this._private__isSettingSize || this._private__state === null) return;
	...
	_internal_destroy() {
		if (this._private__leftPriceAxisWidget !== null) this._private__leftPriceAxisWidget._internal_destroy();
		if (this._private__rightPriceAxisWidget !== null) this._private__rightPriceAxisWidget._internal_destroy();
		this._private__attributionLogoWidget = null;
		this._private__topCanvasBinding.unsubscribeSuggestedBitmapSizeChanged(this._private__topCanvasSuggestedBitmapSizeChangedHandler);
		releaseCanvas(this._private__topCanvasBinding.canvasElement);
		this._private__topCanvasBinding.dispose();
		...
		releaseCanvas(this._private__canvasBinding.canvasElement);
		this._private__canvasBinding.dispose();
		if (this._private__state !== null) this._private__state._internal_onDestroyed()._internal_unsubscribeAll(this);
		this._private__mouseEventHandler._internal_destroy();
	}
	_internal_state() {
	...
	_internal_setState(pane) {
		...
		if (this._private__state !== null) this._private__state._internal_onDestroyed()._internal_subscribe(PaneWidget.prototype._private__onStateDestroyed.bind(this), this, true);
		this._internal_updatePriceAxisWidgetsStates();
		if (this._private__chart._internal_paneWidgets().indexOf(this) === this._private__chart._internal_paneWidgets().length - 1) {
			...
			(_b = this._private__attributionLogoWidget) === null || _b === void 0 || _b._internal_removeElement();
			this._private__attributionLogoWidget = null;
		}
	}
	_internal_chart() {
	...
	_internal_updatePriceAxisWidgetsStates() {
		if (this._private__state === null) return;
		this._private__recreatePriceAxisWidgets();
		if (this._private__model()._internal_serieses().length === 0) return;
	...
	_internal_updatePriceAxisWidgets() {
		if (this._private__leftPriceAxisWidget !== null) this._private__leftPriceAxisWidget._internal_update();
		if (this._private__rightPriceAxisWidget !== null) this._private__rightPriceAxisWidget._internal_update();
	}
	_internal_stretchFactor() {
		return this._private__state !== null ? this._private__state._internal_stretchFactor() : 0;
	}
	_internal_setStretchFactor(stretchFactor) {
		if (this._private__state) this._private__state._internal_setStretchFactor(stretchFactor);
	}
	_internal_mouseEnterEvent(event) {
		if (!this._private__state) return;
	...
	_internal_pinchEvent(middlePoint, scale) {
		if (!this._private__chart._internal_options().handleScale.pinch) return;
	...
	_internal_touchEndEvent(event) {
		if (this._internal_chart()._internal_options().trackingMode.exitMode === 0) this._private__exitTrackingModeOnNextTry = true;
	...
	_internal_setPriceAxisSize(width, position) {
		ensureNotNull(position === "left" ? this._private__leftPriceAxisWidget : this._private__rightPriceAxisWidget)._internal_setSize(size$1({
			width,
	...
	_internal_setSize(newSize) {
		if (equalSizes(this._private__size, newSize)) return;
		this._private__size = newSize;
		this._private__isSettingSize = true;
		this._private__canvasBinding.resizeCanvasElement(newSize);
		this._private__topCanvasBinding.resizeCanvasElement(newSize);
	...
	_internal_recalculatePriceScales() {
		...
		for (const source of pane._internal_dataSources()) if (pane._internal_isOverlay(source)) {
			const priceScale = source._internal_priceScale();
			if (priceScale !== null) pane._internal_recalculatePriceScale(priceScale);
			source._internal_updateAllViews();
		}
	}
	_internal_getBitmapSize() {
	...
	_internal_paint(type) {
		if (type === 0) return;
		if (this._private__state === null) return;
		if (type > 1) this._internal_recalculatePriceScales();
		if (this._private__leftPriceAxisWidget !== null) this._private__leftPriceAxisWidget._internal_paint(type);
		if (this._private__rightPriceAxisWidget !== null) this._private__rightPriceAxisWidget._internal_paint(type);
		if (type !== 1) {
			this._private__canvasBinding.applySuggestedBitmapSize();
			const target = tryCreateCanvasRenderingTarget2D(this._private__canvasBinding);
			if (target !== null) {
	...
	_private__fireMouseClickDelegate(delegate, event) {
		const x = event.localX;
		const y = event.localY;
		if (delegate._internal_hasListeners()) delegate._internal_fire(this._private__model()._internal_timeScale()._internal_coordinateToIndex(x), {
			x,
			y
		}, event);
	}
	_private__drawBackground({ context: ctx, bitmapSize }) {
		...
		const topColor = model._internal_backgroundTopColor();
		const bottomColor = model._internal_backgroundBottomColor();
		if (topColor === bottomColor) clearRect(ctx, 0, 0, width, height, bottomColor);
		else clearRectWithGradient(ctx, 0, 0, width, height, topColor, bottomColor);
	}
	_private__drawGrid(target) {
		const renderer = ensureNotNull(this._private__state)._internal_grid()._internal_paneView()._internal_renderer();
		if (renderer !== null) renderer._internal_draw(target, false);
	}
	...
	_private__drawSources(target, paneViewsGetter) {
		const sources = ensureNotNull(this._private__state)._internal_orderedSources();
		for (const source of sources) this._private__drawSourceImpl(target, paneViewsGetter, drawBackground, source);
		for (const source of sources) this._private__drawSourceImpl(target, paneViewsGetter, drawForeground, source);
	}
	...
	_private__endScroll(event) {
		if (!this._private__isScrolling) return;
		const model = this._private__model();
		...
		if (this._private__scrollXAnimation !== null) {
			...
			if (!this._private__scrollXAnimation._internal_finished(startAnimationTime)) model._internal_setTimeScaleAnimation(this._private__scrollXAnimation);
		}
	}
	...
	_private__mouseTouchDownEvent() {
		if (!this._private__state) return;
		this._private__model()._internal_stopTimeScaleAnimation();
		if (document.activeElement !== document.body && document.activeElement !== document.documentElement) ensureNotNull(document.activeElement).blur();
		else {
			const selection = document.getSelection();
			if (selection !== null) selection.removeAllRanges();
		}
		if (this._private__state._internal_defaultPriceScale()._internal_isEmpty() || this._private__model()._internal_timeScale()._internal_isEmpty()) return;
	}
	_private__pressedMouseTouchMoveEvent(event) {
		...
		if ((!scrollOptions.pressedMouseMove || event._internal_isTouch) && (!scrollOptions.horzTouchDrag && !scrollOptions.vertTouchDrag || !event._internal_isTouch)) return;
		const priceScale = this._private__state._internal_defaultPriceScale();
		const now = performance.now();
		if (this._private__startScrollingPos === null && !this._private__preventScroll(event)) this._private__startScrollingPos = {
			x: event.clientX,
			y: event.clientY,
		...
		if (this._private__startScrollingPos !== null && !this._private__isScrolling && (this._private__startScrollingPos.x !== event.clientX || this._private__startScrollingPos.y !== event.clientY)) {
			if (event._internal_isTouch && kineticScrollOptions.touch || !event._internal_isTouch && kineticScrollOptions.mouse) {
				...
				this._private__scrollXAnimation = new KineticAnimation(.2 / barSpacing, 7 / barSpacing, .997, 15 / barSpacing);
			...
			} else this._private__scrollXAnimation = null;
			if (!priceScale._internal_isEmpty()) model._internal_startScrollPrice(this._private__state, priceScale, event.localY);
			model._internal_startScrollTime(event.localX);
			this._private__isScrolling = true;
		}
		if (this._private__isScrolling) {
			if (!priceScale._internal_isEmpty()) model._internal_scrollPriceTo(this._private__state, priceScale, event.localY);
			model._internal_scrollTimeTo(event.localX);
			if (this._private__scrollXAnimation !== null) this._private__scrollXAnimation._internal_addPosition(timeScale._internal_rightOffset(), now);
		}
	}
};
var PriceAxisStub = class {
	constructor(side, options, params, borderVisible, bottomColor) {
		...
		this._private__canvasSuggestedBitmapSizeChangedHandler = () => this._internal_paint(3);
		this._private__isLeft = side === "left";
	...
	_internal_paint(type) {
		if (type < 3 && !this._private__invalidated) return;
		if (this._private__size.width === 0 || this._private__size.height === 0) return;
		this._private__invalidated = false;
		this._private__canvasBinding.applySuggestedBitmapSize();
		const target = tryCreateCanvasRenderingTarget2D(this._private__canvasBinding);
		if (target !== null) target.useBitmapCoordinateSpace((scope) => {
			this._private__drawBackground(scope);
			this._private__drawBorder(scope);
		});
	}
	...
	_private__drawBorder({ context: ctx, bitmapSize, horizontalPixelRatio, verticalPixelRatio }) {
		if (!this._private__borderVisible()) return;
...
var TimeAxisWidget = class {
	constructor(chartWidget, horzScaleBehavior) {
		...
		this._private__isSettingSize = false;
		this._private__canvasSuggestedBitmapSizeChangedHandler = () => {
			if (!this._private__isSettingSize) this._private__chart._internal_model()._internal_lightUpdate();
		};
		this._private__topCanvasSuggestedBitmapSizeChangedHandler = () => {
			if (!this._private__isSettingSize) this._private__chart._internal_model()._internal_lightUpdate();
		};
	...
	_internal_destroy() {
		this._private__mouseEventHandler._internal_destroy();
		if (this._private__leftStub !== null) this._private__leftStub._internal_destroy();
		if (this._private__rightStub !== null) this._private__rightStub._internal_destroy();
		this._private__topCanvasBinding.unsubscribeSuggestedBitmapSizeChanged(this._private__topCanvasSuggestedBitmapSizeChangedHandler);
		releaseCanvas(this._private__topCanvasBinding.canvasElement);
		this._private__topCanvasBinding.dispose();
		this._private__canvasBinding.unsubscribeSuggestedBitmapSizeChanged(this._private__canvasSuggestedBitmapSizeChangedHandler);
	...
	_internal_mouseDownEvent(event) {
		if (this._private__mouseDown) return;
		this._private__mouseDown = true;
		const model = this._private__chart._internal_model();
		if (model._internal_timeScale()._internal_isEmpty() || !this._private__chart._internal_options().handleScale.axisPressedMouseMove.time) return;
	...
	_internal_mouseDownOutsideEvent() {
		...
		if (!model._internal_timeScale()._internal_isEmpty() && this._private__mouseDown) {
			this._private__mouseDown = false;
			if (this._private__chart._internal_options().handleScale.axisPressedMouseMove.time) model._internal_endScaleTime();
		}
	}
	_internal_pressedMouseMoveEvent(event) {
		const model = this._private__chart._internal_model();
	...
	_internal_mouseDoubleClickEvent() {
		if (this._private__chart._internal_options().handleScale.axisDoubleClickReset.time) this._private__chart._internal_model()._internal_resetTimeScale();
	}
	_internal_doubleTapEvent() {
		this._internal_mouseDoubleClickEvent();
	}
	_internal_mouseEnterEvent() {
		if (this._private__chart._internal_model()._internal_options().handleScale.axisPressedMouseMove.time) this._private__setCursor(1);
	}
	...
	_internal_setSizes(timeAxisSize, leftStubWidth, rightStubWidth) {
		...
		if (this._private__leftStub !== null) this._private__leftStub._internal_setSize(size$1({
			width: leftStubWidth,
			height: timeAxisSize.height
		}));
		if (this._private__rightStub !== null) this._private__rightStub._internal_setSize(size$1({
	...
	_internal_optimalHeight() {
		...
		return Math.ceil(rendererOptions._internal_borderSize + rendererOptions._internal_tickLength + rendererOptions._internal_fontSize + rendererOptions._internal_paddingTop + rendererOptions._internal_paddingBottom + rendererOptions._internal_labelBottomOffset);
	}
	_internal_update() {
	...
	_internal_paint(type) {
		...
		if (type !== 1) {
			...
			if (this._private__leftStub !== null) this._private__leftStub._internal_paint(type);
			if (this._private__rightStub !== null) this._private__rightStub._internal_paint(type);
		}
		this._private__topCanvasBinding.applySuggestedBitmapSize();
		const topTarget = tryCreateCanvasRenderingTarget2D(this._private__topCanvasBinding);
		if (topTarget !== null) {
	...
	_private__drawAdditionalSources(target, axisViewsGetter) {
		...
		for (const source of sources) drawSourcePaneViews(axisViewsGetter, (renderer) => drawBackground(renderer, target, false, void 0), source, void 0);
		for (const source of sources) drawSourcePaneViews(axisViewsGetter, (renderer) => drawForeground(renderer, target, false, void 0), source, void 0);
	}
	_private__drawBackground({ context: ctx, bitmapSize }) {
	...
	_private__drawTickMarks(target) {
		...
		if (options.borderVisible && options.ticksVisible) target.useBitmapCoordinateSpace(({ context: ctx, horizontalPixelRatio, verticalPixelRatio }) => {
		...
		target.useMediaCoordinateSpace(({ context: ctx }) => {
			...
			ctx.font = this._private__baseFont();
			for (const tickMark of tickMarks) if (tickMark.weight < maxWeight) {
			...
			if (this._private__chart._internal_options().timeScale.allowBoldLabels) ctx.font = this._private__baseBoldFont();
			for (const tickMark of tickMarks) if (tickMark.weight >= maxWeight) {
				const coordinate = tickMark.needAlignCoordinate ? this._private__alignTickMarkLabelCoordinate(ctx, tickMark.coord, tickMark.label) : tickMark.coord;
				ctx.fillText(tickMark.label, coordinate, yText);
			}
	...
	_private__alignTickMarkLabelCoordinate(ctx, coordinate, labelText) {
		...
		const labelWidthHalf = labelWidth / 2;
		const leftTextCoordinate = Math.floor(coordinate - labelWidthHalf) + .5;
		if (leftTextCoordinate < 0) coordinate = coordinate + Math.abs(0 - leftTextCoordinate);
		else if (leftTextCoordinate + labelWidth > this._private__size.width) coordinate = coordinate - Math.abs(this._private__size.width - (leftTextCoordinate + labelWidth));
		return coordinate;
	}
	_private__drawLabels(sources, target) {
		const rendererOptions = this._private__getRendererOptions();
		for (const source of sources) for (const view of source._internal_timeAxisViews()) view._internal_renderer()._internal_draw(target, rendererOptions);
	}
	_private__lineColor() {
	...
	_private__getRendererOptions() {
		if (this._private__rendererOptions === null) this._private__rendererOptions = {
			_internal_borderSize: 1,
	...
	_private__recreateStubs() {
		...
		const params = { _internal_rendererOptionsProvider: this._private__chart._internal_model()._internal_rendererOptionsProvider() };
...
var ChartWidget = class {
	constructor(container, options, horzScaleBehavior) {
		...
		if (shouldSubscribeMouseWheel(this._private__options)) this._private__setMouseWheelEventListener(true);
	...
	_internal_destroy() {
		...
		if (this._private__drawRafId !== 0) window.cancelAnimationFrame(this._private__drawRafId);
		...
		if (this._private__element.parentElement !== null) this._private__element.parentElement.removeChild(this._private__element);
	...
	_internal_resize(width, height, forceRepaint = false) {
		if (this._private__height === height && this._private__width === width) return;
		...
		if (forceRepaint) this._private__drawImpl(InvalidateMask._internal_full(), performance.now());
		else this._private__model._internal_fullUpdate();
	}
	_internal_paint(invalidateMask) {
		if (invalidateMask === void 0) invalidateMask = InvalidateMask._internal_full();
		for (let i = 0; i < this._private__paneWidgets.length; i++) this._private__paneWidgets[i]._internal_paint(invalidateMask._internal_invalidateForPane(i)._internal_level);
		if (this._private__options.timeScale.visible) this._private__timeAxisWidget._internal_paint(invalidateMask._internal_fullInvalidation());
	}
	_internal_applyOptions(options) {
		...
		if (shouldHaveMouseWheelListener !== currentlyHasMouseWheelListener) this._private__setMouseWheelEventListener(shouldHaveMouseWheelListener);
		this._private__updateTimeAxisVisibility();
	...
	_internal_getPriceAxisWidth(position) {
		if (position === "left" && !this._private__isLeftAxisVisible()) return 0;
		if (position === "right" && !this._private__isRightAxisVisible()) return 0;
		if (this._private__paneWidgets.length === 0) return 0;
		return ensureNotNull(position === "left" ? this._private__paneWidgets[0]._internal_leftPriceAxisWidget() : this._private__paneWidgets[0]._internal_rightPriceAxisWidget())._internal_getWidth();
	}
	...
	_internal_setCursorStyle(style) {
		...
		if (this._private__cursorStyleOverride) this._internal_element().style.setProperty("cursor", style);
		else this._internal_element().style.removeProperty("cursor");
	}
	...
	_private__applyAutoSizeOptions(options) {
		...
		if (options.autoSize && !this._private__observer) this._private__installObserver();
		if (options.autoSize === false && this._private__observer !== null) this._private__uninstallObserver();
		if (!options.autoSize && (options.width !== void 0 || options.height !== void 0)) this._internal_resize(options.width || this._private__width, options.height || this._private__height);
	}
	/**
	...
	*/
	_private__traverseLayout(ctx) {
		...
		const drawPriceAxises = (position, targetX) => {
			...
			for (let paneIndex = 0; paneIndex < this._private__paneWidgets.length; paneIndex++) {
				...
				if (ctx !== null) priceAxisWidget._internal_drawBitmap(ctx, targetX, targetY);
		...
		for (let paneIndex = 0; paneIndex < this._private__paneWidgets.length; paneIndex++) {
			...
			if (ctx !== null) paneWidget._internal_drawBitmap(ctx, totalWidth, totalHeight);
		...
		const drawStub = (position, targetX, targetY) => {
			ensureNotNull(position === "left" ? this._private__timeAxisWidget._internal_leftStub() : this._private__timeAxisWidget._internal_rightStub())._internal_drawBitmap(ensureNotNull(ctx), targetX, targetY);
		};
		if (this._private__options.timeScale.visible) {
			...
			if (ctx !== null) {
				...
				if (this._private__isRightAxisVisible()) drawStub("right", targetX, totalHeight);
			}
	...
	_private__adjustSizeImpl() {
		...
		for (const paneWidget of this._private__paneWidgets) {
			if (this._private__isLeftAxisVisible()) leftPriceAxisWidth = Math.max(leftPriceAxisWidth, ensureNotNull(paneWidget._internal_leftPriceAxisWidget())._internal_optimalWidth(), this._private__options.leftPriceScale.minimumWidth);
			if (this._private__isRightAxisVisible()) rightPriceAxisWidth = Math.max(rightPriceAxisWidth, ensureNotNull(paneWidget._internal_rightPriceAxisWidget())._internal_optimalWidth(), this._private__options.rightPriceScale.minimumWidth);
		...
		for (let paneIndex = 0; paneIndex < this._private__paneWidgets.length; ++paneIndex) {
			...
			if (paneIndex === this._private__paneWidgets.length - 1) calculatePaneHeight = totalPaneHeight - accumulatedHeight;
			else calculatePaneHeight = Math.round(paneWidget._internal_stretchFactor() * stretchPixels);
			...
			if (this._private__isLeftAxisVisible()) paneWidget._internal_setPriceAxisSize(leftPriceAxisWidth, "left");
			if (this._private__isRightAxisVisible()) paneWidget._internal_setPriceAxisSize(rightPriceAxisWidth, "right");
			if (paneWidget._internal_state()) this._private__model._internal_setPaneHeight(paneWidget._internal_state(), paneHeight);
		}
		...
		if (this._private__leftPriceAxisWidth !== leftPriceAxisWidth) this._private__leftPriceAxisWidth = leftPriceAxisWidth;
		if (this._private__rightPriceAxisWidth !== rightPriceAxisWidth) this._private__rightPriceAxisWidth = rightPriceAxisWidth;
	}
	...
	_private__determineWheelSpeedAdjustment(event) {
		switch (event.deltaMode) {
			case event.DOM_DELTA_PAGE: return 120;
			case event.DOM_DELTA_LINE: return 32;
		}
		if (!windowsChrome) return 1;
		return 1 / window.devicePixelRatio;
	}
	_private__onMousewheel(event) {
		if ((event.deltaX === 0 || !this._private__options.handleScroll.mouseWheel) && (event.deltaY === 0 || !this._private__options.handleScale.mouseWheel)) return;
		...
		if (deltaX !== 0 && this._private__options.handleScroll.mouseWheel) this._internal_model()._internal_scrollChart(deltaX * -80);
	}
	_private__drawImpl(invalidateMask, time) {
		var _a;
		const invalidationType = invalidateMask._internal_fullInvalidation();
		if (invalidationType === 3) this._private__updateGui();
	...
	_private__applyTimeScaleInvalidations(invalidateMask, time) {
		for (const tsInvalidation of invalidateMask._internal_timeScaleInvalidations()) this._private__applyTimeScaleInvalidation(tsInvalidation, time);
	}
	_private__applyMomentaryAutoScale(invalidateMask) {
		const panes = this._private__model._internal_panes();
		for (let i = 0; i < panes.length; i++) if (invalidateMask._internal_invalidateForPane(i)._internal_autoScale) panes[i]._internal_momentaryAutoScale();
	}
	_private__applyTimeScaleInvalidation(invalidation, time) {
		...
		switch (invalidation._internal_type) {
			...
			case 4:
				...
				break;
			case 5:
				if (!invalidation._internal_value._internal_finished(time)) timeScale._internal_setRightOffset(invalidation._internal_value._internal_getPosition(time));
				break;
		}
	}
	_private__invalidateHandler(invalidateMask) {
		if (this._private__invalidateMask !== null) this._private__invalidateMask._internal_merge(invalidateMask);
		else this._private__invalidateMask = invalidateMask;
		if (!this._private__drawPlanned) {
			...
			this._private__drawRafId = window.requestAnimationFrame((time) => {
				...
				if (this._private__invalidateMask !== null) {
					...
					for (const tsInvalidation of mask._internal_timeScaleInvalidations()) if (tsInvalidation._internal_type === 5 && !tsInvalidation._internal_value._internal_finished(time)) {
	...
	_private__syncGuiWithModel() {
		...
		for (let i = 0; i < targetPaneWidgetsCount; i++) {
			...
			if (paneWidget._internal_state() !== state) paneWidget._internal_setState(state);
			else paneWidget._internal_updatePriceAxisWidgetsStates();
		}
		this._private__updateTimeAxisVisibility();
	...
	_private__getMouseEventParamsImpl(index, point, event) {
		...
		if (index !== null) this._private__model._internal_serieses().forEach((s) => {
			const data = s._internal_bars()._internal_search(index);
			if (data !== null) seriesData.set(s, data);
		});
		let clientTime;
		if (index !== null) {
			...
			if (timePoint !== void 0) clientTime = timePoint;
		}
	...
	_private__uninstallObserver() {
		if (this._private__observer !== null) this._private__observer.disconnect();
		this._private__observer = null;
	}
};
...
function getColoredLineBasedSeriesPlotRow(time, index, item, originalTime) {
	...
	if (item.color !== void 0) res._internal_color = item.color;
	return res;
}
function getAreaSeriesPlotRow(time, index, item, originalTime) {
	...
	if (item.lineColor !== void 0) res._internal_lineColor = item.lineColor;
	if (item.topColor !== void 0) res._internal_topColor = item.topColor;
	if (item.bottomColor !== void 0) res._internal_bottomColor = item.bottomColor;
	return res;
}
function getBaselineSeriesPlotRow(time, index, item, originalTime) {
	...
	if (item.topLineColor !== void 0) res._internal_topLineColor = item.topLineColor;
	if (item.bottomLineColor !== void 0) res._internal_bottomLineColor = item.bottomLineColor;
	if (item.topFillColor1 !== void 0) res._internal_topFillColor1 = item.topFillColor1;
	if (item.topFillColor2 !== void 0) res._internal_topFillColor2 = item.topFillColor2;
	if (item.bottomFillColor1 !== void 0) res._internal_bottomFillColor1 = item.bottomFillColor1;
	if (item.bottomFillColor2 !== void 0) res._internal_bottomFillColor2 = item.bottomFillColor2;
	return res;
}
...
function getCandlestickSeriesPlotRow(time, index, item, originalTime) {
	...
	if (item.color !== void 0) res._internal_color = item.color;
	if (item.borderColor !== void 0) res._internal_borderColor = item.borderColor;
	if (item.wickColor !== void 0) res._internal_wickColor = item.wickColor;
	return res;
}
function getCustomSeriesPlotRow(time, index, item, originalTime, dataToPlotRow) {
	...
	const _a = item, { time: excludedTime, color } = _a;
	return {
		_internal_index: index,
		_internal_time: time,
		_internal_value: value,
		_internal_originalTime: originalTime,
		_internal_data: __rest(_a, ["time", "color"]),
...
function wrapCustomValues(plotRow, bar) {
	if (bar.customValues !== void 0) plotRow._internal_customValues = bar.customValues;
	return plotRow;
}
function isWhitespaceDataWithCustomCheck(bar, customIsWhitespace) {
	if (customIsWhitespace) return customIsWhitespace(bar);
...
function wrapWhitespaceData(createPlotRowFn) {
	return (time, index, bar, originalTime, dataToPlotRow, customIsWhitespace) => {
		if (isWhitespaceDataWithCustomCheck(bar, customIsWhitespace)) return wrapCustomValues({
			_internal_time: time,
			_internal_index: index,
			_internal_originalTime: originalTime
		}, bar);
...
function createEmptyTimePointData(timePoint) {
	return {
		_internal_index: 0,
		_internal_mapping: /* @__PURE__ */ new Map(),
		_internal_timePoint: timePoint
	};
}
function seriesRowsFirstAndLastTime(seriesRows, bh) {
	if (seriesRows === void 0 || seriesRows.length === 0) return;
	return {
...
function seriesUpdateInfo(seriesRows, prevSeriesRows, bh) {
	...
	if (firstAndLastTime !== void 0 && prevFirstAndLastTime !== void 0) return { _internal_lastBarUpdatedOrNewBarsAddedToTheRight: firstAndLastTime._internal_lastTime >= prevFirstAndLastTime._internal_lastTime && firstAndLastTime._internal_firstTime >= prevFirstAndLastTime._internal_firstTime };
}
function timeScalePointTime(mergedPointData) {
	let result;
	mergedPointData.forEach((v) => {
		if (result === void 0) result = v._internal_originalTime;
	});
	return ensureDefined(result);
}
function saveOriginalTime(data) {
	if (data._internal_originalTime === void 0) data._internal_originalTime = data.time;
}
var DataLayer = class {
	...
	_internal_setSeriesData(series, data) {
		...
		if (prevSeriesRows !== void 0) {
			if (this._private__seriesRowsBySeries.size === 1) {
				...
				this._private__pointDataByTimePoint.clear();
			} else for (const point of this._private__sortedTimePoints) if (point.pointData._internal_mapping.delete(series)) isTimeScaleAffected = true;
		}
		let seriesRows = [];
		if (data.length !== 0) {
		...
		if (needCleanupPoints) this._private__cleanupPointsData();
	...
	_internal_updateSeriesData(series, data) {
		...
		const time = this._private__horzScaleBehavior.createConverterToInternalObj([data])(data.time);
		const lastSeriesTime = this._private__seriesLastTimePoint.get(series);
		if (lastSeriesTime !== void 0 && this._private__horzScaleBehavior.key(time) < this._private__horzScaleBehavior.key(lastSeriesTime)) throw new Error(`Cannot update oldest data, last time=${lastSeriesTime}, new time=${time}`);
		...
		if (!affectsTimeScale) return this._private__getUpdateResponse(series, -1, info);
		const newPoint = {
			timeWeight: 0,
		...
		for (let index = insertIndex; index < this._private__sortedTimePoints.length; ++index) assignIndexToPointData(this._private__sortedTimePoints[index].pointData, index);
	...
	_private__updateLastSeriesRow(series, plotRow) {
		...
		if (lastSeriesRow === null || this._private__horzScaleBehavior.key(plotRow._internal_time) > this._private__horzScaleBehavior.key(lastSeriesRow._internal_time)) {
			if (isSeriesPlotRow(plotRow)) seriesData.push(plotRow);
		} else if (isSeriesPlotRow(plotRow)) seriesData[seriesData.length - 1] = plotRow;
		else seriesData.splice(-1, 1);
	...
	_private__cleanupPointsData() {
		for (const point of this._private__sortedTimePoints) if (point.pointData._internal_mapping.size === 0) this._private__pointDataByTimePoint.delete(this._private__horzScaleBehavior.key(point.time));
	}
	/**
	...
	*/
	_private__replaceTimeScalePoints(newTimePoints) {
		...
		if (firstChangedPointIndex === -1 && this._private__sortedTimePoints.length !== newTimePoints.length) firstChangedPointIndex = Math.min(this._private__sortedTimePoints.length, newTimePoints.length);
		if (firstChangedPointIndex === -1) return -1;
		for (let index = firstChangedPointIndex; index < newTimePoints.length; ++index) assignIndexToPointData(newTimePoints[index].pointData, index);
	...
	_private__getBaseIndex() {
		if (this._private__seriesRowsBySeries.size === 0) return null;
		let baseIndex = 0;
		this._private__seriesRowsBySeries.forEach((data) => {
			if (data.length !== 0) baseIndex = Math.max(baseIndex, data[data.length - 1]._internal_index);
		});
		return baseIndex;
	}
	_private__getUpdateResponse(updatedSeries, firstChangedPointIndex, info) {
		const dataUpdateResponse = {
			...
			_internal_timeScale: { _internal_baseIndex: this._private__getBaseIndex() }
		};
		if (firstChangedPointIndex !== -1) {
			...
			if (!this._private__seriesRowsBySeries.has(updatedSeries)) dataUpdateResponse._internal_series.set(updatedSeries, {
				_internal_data: [],
				_internal_info: info
			});
...
function singleValueData(plotRow) {
	const data = {
		value: plotRow._internal_value[3],
		time: plotRow._internal_originalTime
	};
	if (plotRow._internal_customValues !== void 0) data.customValues = plotRow._internal_customValues;
	return data;
}
function lineData(plotRow) {
	const result = singleValueData(plotRow);
	if (plotRow._internal_color !== void 0) result.color = plotRow._internal_color;
	return result;
}
function areaData(plotRow) {
	const result = singleValueData(plotRow);
	if (plotRow._internal_lineColor !== void 0) result.lineColor = plotRow._internal_lineColor;
	if (plotRow._internal_topColor !== void 0) result.topColor = plotRow._internal_topColor;
	if (plotRow._internal_bottomColor !== void 0) result.bottomColor = plotRow._internal_bottomColor;
	return result;
}
function baselineData(plotRow) {
	const result = singleValueData(plotRow);
	if (plotRow._internal_topLineColor !== void 0) result.topLineColor = plotRow._internal_topLineColor;
	if (plotRow._internal_bottomLineColor !== void 0) result.bottomLineColor = plotRow._internal_bottomLineColor;
	if (plotRow._internal_topFillColor1 !== void 0) result.topFillColor1 = plotRow._internal_topFillColor1;
	if (plotRow._internal_topFillColor2 !== void 0) result.topFillColor2 = plotRow._internal_topFillColor2;
	if (plotRow._internal_bottomFillColor1 !== void 0) result.bottomFillColor1 = plotRow._internal_bottomFillColor1;
	if (plotRow._internal_bottomFillColor2 !== void 0) result.bottomFillColor2 = plotRow._internal_bottomFillColor2;
	return result;
}
function ohlcData(plotRow) {
	const data = {
		open: plotRow._internal_value[0],
		high: plotRow._internal_value[1],
		low: plotRow._internal_value[2],
		close: plotRow._internal_value[3],
		time: plotRow._internal_originalTime
	};
	if (plotRow._internal_customValues !== void 0) data.customValues = plotRow._internal_customValues;
	return data;
...
function candlestickData(plotRow) {
	...
	if (borderColor !== void 0) result.borderColor = borderColor;
	if (wickColor !== void 0) result.wickColor = wickColor;
	return result;
}
...
var PriceScaleApi = class {
	...
	width() {
		if (!isDefaultPriceScale(this._private__priceScaleId)) return 0;
...
function checkItemsAreOrdered(data, bh, allowDuplicates = false) {
	...
	for (let i = 1; i < data.length; ++i) {
		...
		assert(allowDuplicates ? prevTime <= currentTime : prevTime < currentTime, `data must be asc ordered by time, index=${i}, time=${currentTime}, prev time=${prevTime}`);
...
function getChecker(type) {
	switch (type) {
		case "Bar":
		case "Candlestick": return checkBarItem.bind(null, type);
		case "Area":
		case "Baseline":
		case "Line":
		case "Histogram": return checkLineItem.bind(null, type);
		case "Custom": return checkCustomItem.bind(null, type);
	}
}
function checkBarItem(type, barItem) {
	if (!isFulfilledBarData(barItem)) return;
...
function checkLineItem(type, lineItem) {
	if (!isFulfilledLineData(lineItem)) return;
...
function convertSeriesMarker(sm, newTime, originalTime) {
	...
	if (originalTime !== void 0) res.originalTime = originalTime;
	return res;
}
...
var SeriesApi = class {
	...
	barsInLogicalRange(range) {
		...
		if (bars._internal_isEmpty()) return null;
		const dataFirstBarInRange = bars._internal_search(correctedRange._internal_left(), 1);
		const dataLastBarInRange = bars._internal_search(correctedRange._internal_right(), -1);
		const dataFirstIndex = ensureNotNull(bars._internal_firstIndex());
		const dataLastIndex = ensureNotNull(bars._internal_lastIndex());
		if (dataFirstBarInRange !== null && dataLastBarInRange !== null && dataFirstBarInRange._internal_index > dataLastBarInRange._internal_index) return {
			barsBefore: range.from - dataFirstIndex,
			barsAfter: dataLastIndex - range.to
		};
		const result = {
			barsBefore: dataFirstBarInRange === null || dataFirstBarInRange._internal_index === dataFirstIndex ? range.from - dataFirstIndex : dataFirstBarInRange._internal_index - dataFirstIndex,
			barsAfter: dataLastBarInRange === null || dataLastBarInRange._internal_index === dataLastIndex ? dataLastIndex - range.to : dataLastIndex - dataLastBarInRange._internal_index
		};
	...
	data() {
		...
		return this._internal__series._internal_bars()._internal_rows().map((row) => seriesCreator(row));
	}
	...
	createPriceLine(options) {
		...
		return new PriceLine(this._internal__series._internal_createPriceLine(strictOptions));
	}
	...
	attachPrimitive(primitive) {
		...
		if (primitive.attached) primitive.attached({
	...
	detachPrimitive(primitive) {
		...
		if (primitive.detached) primitive.detached();
	}
	_private__onDataChanged(scope) {
		if (this._private__dataChangedDelegate._internal_hasListeners()) this._private__dataChangedDelegate._internal_fire(scope);
	}
};
var TimeScaleApi = class {
	...
	scrollToPosition(position, animated) {
		...
		this._private__timeScale._internal_scrollToOffsetAnimated(position, 1e3);
	}
	scrollToRealTime() {
	...
	getVisibleLogicalRange() {
		...
		if (logicalRange === null) return null;
		return {
	...
	logicalToCoordinate(logical) {
		const timeScale = this._private__model._internal_timeScale();
		if (timeScale._internal_isEmpty()) return null;
		else return timeScale._internal_indexToCoordinate(logical);
	}
	coordinateToLogical(x) {
		if (this._private__timeScale._internal_isEmpty()) return null;
		else return this._private__timeScale._internal_coordinateToIndex(x);
	}
	timeToCoordinate(time) {
		...
		if (timePointIndex === null) return null;
	...
	_private__onVisibleBarsChanged() {
		if (this._private__timeRangeChanged._internal_hasListeners()) this._private__timeRangeChanged._internal_fire(this.getVisibleRange());
	}
	_private__onVisibleLogicalRangeChanged() {
		if (this._private__logicalRangeChanged._internal_hasListeners()) this._private__logicalRangeChanged._internal_fire(this.getVisibleLogicalRange());
	}
...
function patchPriceFormat(priceFormat) {
	if (priceFormat === void 0 || priceFormat.type === "custom") return;
	const priceFormatBuiltIn = priceFormat;
	if (priceFormatBuiltIn.minMove !== void 0 && priceFormatBuiltIn.precision === void 0) priceFormatBuiltIn.precision = precisionByMinMove(priceFormatBuiltIn.minMove);
}
function migrateHandleScaleScrollOptions(options) {
	if (isBoolean(options.handleScale)) {
		...
		if (isBoolean(axisPressedMouseMove)) options.handleScale.axisPressedMouseMove = {
	...
	if (isBoolean(handleScroll)) options.handleScroll = {
...
var ChartApi = class {
	constructor(container, horzScaleBehavior, options) {
		...
		this._private__chartWidget._internal_clicked()._internal_subscribe((paramSupplier) => {
			if (this._private__clickedDelegate._internal_hasListeners()) this._private__clickedDelegate._internal_fire(this._private__convertMouseParams(paramSupplier()));
		}, this);
		this._private__chartWidget._internal_dblClicked()._internal_subscribe((paramSupplier) => {
			if (this._private__dblClickedDelegate._internal_hasListeners()) this._private__dblClickedDelegate._internal_fire(this._private__convertMouseParams(paramSupplier()));
		}, this);
		this._private__chartWidget._internal_crosshairMoved()._internal_subscribe((paramSupplier) => {
			if (this._private__crosshairMovedDelegate._internal_hasListeners()) this._private__crosshairMovedDelegate._internal_fire(this._private__convertMouseParams(paramSupplier()));
		}, this);
		this._private__timeScaleApi = new TimeScaleApi(this._private__chartWidget._internal_model(), this._private__chartWidget._internal_timeAxisWidget(), this._private__horzScaleBehavior);
	}
	remove() {
	...
	removeSeries(seriesApi) {
		...
		this._private__chartWidget._internal_model()._internal_removeSeries(series);
	...
	_private__convertMouseParams(param) {
		...
		param._internal_seriesData.forEach((plotRow, series) => {
			...
			if (seriesType !== "Custom") assert(isFulfilledData(data));
			else {
				const customWhitespaceChecker = series._internal_customSeriesWhitespaceCheck();
